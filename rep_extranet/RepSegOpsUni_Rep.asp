	...
	Else
		...
		if mov = "i" Then
			header = header & 	celdahead("Descripci&oacute;n de las Mercanc&iacute;as") &_
								...
								celdahead("Tipo de Mercanc&iacute;a") &_
								celdahead("Pais Origen")
		Else
			header = header & 	celdahead("Descripci&oacute;n de las Mercanc&iacute;as") &_
								celdahead("N&uacute;mero de Facturas") &_
								celdahead("Tipo de Mercanc&iacute;a") &_
		...
		End If
		header = header &		celdahead("Tiempos de Despacho Indicador Cliente (Entrada vs Despacho)") &_
		...
		if mov = "i" Then
			...
			header = header & 		Celdahead("Causa del Desv&iacute;o (Arbol de Perdidas)")
		End If
		...
		Do Until RSops.EOF
			...
			datos = datos &	"<tr>" &_
								...
								celdadatos(RSops.Fields.Item("Clvp").Value) &_
								celdadatos(RSops.Fields.Item("OC").Value) &_
								celdadatos(RSops.Fields.Item("nomcli").Value) &_
								celdadatos(RSops.Fields.Item("ejecutivo").Value) &_
								celdadatos(RSops.Fields.Item("B/L").Value)
			if mov = "i" Then
				datos = datos & celdadatos(RSops.Fields.Item("Descpro").Value) &_
								celdadatos(RSops.Fields.Item("Nomproveedor").Value) &_
								celdadatos(RSops.Fields.Item("TM").Value)
			Else
			...
			if mov = "i" Then
				...
				datos = datos & celdadatos(ObservacionesT(Referencia))
				datos = datos &	celdadatos(Causales(Referencia, "T"))
			else
				datos = datos &	celdadatos(ObservacionesT(Referencia))'ausales(Referencia, "%"))
				datos = datos &	celdadatos(Causales(Referencia, "T"))
			end if
			...
			datos = datos &		celdadatos(RSops.Fields.Item("FCG").Value) &_
								...
								if Tiporepo=2 then 
									if mov="i"  and  strOficina<>"rku" then 
										datos=datos&celdadatos("=SI(O(IZQUIERDA(A"&contadorceldas&",3)=""RKU"", IZQUIERDA(A"&contadorceldas&",3)=""SAP"",IZQUIERDA(A"&contadorceldas&",3)=""LZR""),DIAS.LAB(Q"&contadorceldas&",Z"&contadorceldas&")+ENTERO((Z"&contadorceldas&"-DIASEM(Z"&contadorceldas&"-6)-Q"&contadorceldas&"+8)/7)*0.5-1,DIAS.LAB(Q"&contadorceldas&",Z"&contadorceldas&")-SI(O(DIASEM(Q"&contadorceldas&")=1,DIASEM(Q"&contadorceldas&")=7),0,1))")
									elseif mov="i" and strOficina="rku" then 
										datos=datos&celdadatos("=SI(O(IZQUIERDA(A"&contadorceldas&",3)=""RKU"", IZQUIERDA(A"&contadorceldas&",3)=""SAP"",IZQUIERDA(A"&contadorceldas&",3)=""LZR""),DIAS.LAB(Q"&contadorceldas&",AA"&contadorceldas&")+ENTERO((AA"&contadorceldas&"-DIASEM(AA"&contadorceldas&"-6)-Q"&contadorceldas&"+8)/7)*0.5-1,DIAS.LAB(Q"&contadorceldas&",AA"&contadorceldas&")-SI(O(DIASEM(Q"&contadorceldas&")=1,DIASEM(Q"&contadorceldas&")=7),0,1))")
									elseif mov="e" and 	strOficina<>"rku" then 
										datos=datos&celdadatos("=SI(O(IZQUIERDA(A"&contadorceldas&",3)=""RKU"", IZQUIERDA(A"&contadorceldas&",3)=""SAP"",IZQUIERDA(A"&contadorceldas&",3)=""LZR""),DIAS.LAB(O"&contadorceldas&",U"&contadorceldas&")+ENTERO((U"&contadorceldas&"-DIASEM(U"&contadorceldas&"-6)-O"&contadorceldas&"+8)/7)*0.5-1,DIAS.LAB(O"&contadorceldas&",U"&contadorceldas&")-SI(O(DIASEM(O"&contadorceldas&")=1,DIASEM(O"&contadorceldas&")=7),0,1))")
									elseif mov="e" and 	strOficina="rku" then 
										datos=datos&celdadatos("=SI(O(IZQUIERDA(A"&contadorceldas&",3)=""RKU"", IZQUIERDA(A"&contadorceldas&",3)=""SAP"",IZQUIERDA(A"&contadorceldas&",3)=""LZR""),DIAS.LAB(O"&contadorceldas&",V"&contadorceldas&")+ENTERO((V"&contadorceldas&"-DIASEM(V"&contadorceldas&"-6)-O"&contadorceldas&"+8)/7)*0.5-1,DIAS.LAB(O"&contadorceldas&",V"&contadorceldas&")-SI(O(DIASEM(O"&contadorceldas&")=1,DIASEM(O"&contadorceldas&")=7),0,1))")
									end if
								else
									datos=datos&celdadatos(RSops.Fields.Item("KPIACUSE").Value) 
								end if
								datos=datos& celdadatos(ObservacionesA(Referencia))
			if mov = "i" Then
...
Function ObservacionesT(refe)
Of=mid(refe,1,3)
observa2=""
If Of="ALC" then Of="LZR" end if
if Of="PAN" then Of="DAI" end if
SQLObser=""
SQLObser="select group_concat(distinct observ separator ', ') as Observaciones,'jeje' as ObservacionesX " &_ 
			"from (select a.c_referencia as refe,concat_ws(' ',b.d_nombre,date_format(max(a.f_fecha),'%d/%m/%Y'),a.m_observ) as observ "&_
			"FROM "&Of&"_status.etxpd as a left join "&Of&"_status.etaps as b on b.n_etapa =a.n_etapa " &_
				"where a.c_referencia='"&refe&"' group by b.d_nombre order by a.f_fecha desc) as etapas group by refe"	
	...
	Else
		...
		Do Until RSobser2.EOF = True
			observa2 = RSobser2.Fields.Item("Observaciones").Value 
			RSobser2.MoveNext()
		Loop
	End If
	RSobser2.Close()
	Set RSobser2 = Nothing
	ObservacionesT = observa2
	
end function

Function ObservacionesA(refe)
'response.write("Entre :"&refe)
Of=mid(refe,1,3)
observa2=""
If Of="ALC" then Of="LZR" end if
if Of="PAN" then Of="DAI" end if
SQLObser=""
SQLObser="select group_concat(distinct p.m_observ separator ' | ') as observ from "&Of&"_status.etxpd as p "&_
			"left join "&Of&"_status.c01caus as c on c.c01clavec=p.clavec "&_
			"where  p.c_referencia='"&refe&"'  and c.c01tipoc='A' "
	
			
	Set RSobser2 = Server.CreateObject("ADODB.Recordset")
	Set RSobser2 = ConnStr.Execute(SQLObser)
	If RSobser2.BOF = True And RSObser2.EOF = True Then
		observa2 = "No se encontraron Observaciones Administrativas"
	Else
		RSobser2.MoveFirst()
		Do Until RSobser2.EOF = True
...
function GeneraSQL
	...
	SQL = 	"SELECT  i.refcia01 AS 'referencia', " &_
		...
		"FROM " & strOficina & "_extranet.ssguia04 AS gui "&_
		"WHERE gui.refcia04 = i.refcia01 AND gui.patent04 = i.patent01 AND gui.adusec04 = i.adusec01 "&_
		"GROUP BY gui.refcia04), '') AS 'B/L', "&_
			...
			"IF(c.fpre01 IS NULL OR c.fpre01 = '0000-00-00', if((select ip.cveide11  from " & stroficina & "_extranet.ssiped11 as ip where ip.refcia11 =i.refcia01 and ip.cveide11 ='RO')='RO',DATE_FORMAT(i.fecpag01,'%d-%m-%Y'), 'No Capturada'), DATE_FORMAT(c.fpre01,'%d-%m-%Y')) AS 'fprev',  " &_
	...
	SQL = SQL & kpi("", "c.frev01", "c.fdsp01") & "AS 'KPIGRK', " &_
			...
			"IF(cta.frec31 IS NULL OR cta.frec31 = '0000-00-00', 'No Capturada', DATE_FORMAT(cta.frec31, '%d-%m-%Y')) as 'facusex', " &_
			" (select if(max(x2.frec31) is not null and max(x2.frec31) <> '0000-00-0', DATE_FORMAT(max(x2.frec31), '%d-%m-%Y'),'No capturada') from  " & strOficina & "_extranet.e31cgast as x2 inner join  " & strOficina & "_extranet.d31refer as x3 on x3.cgas31 = x2.cgas31 where x2.esta31 <>'C' and x3.refe31 = i.refcia01) as 'facuse', " & _
			...
			kpi("", "c.fdsp01", "@maxfacu") & "AS 'KPIACUSE', " &_
			...
			"if(c.ejecli01 =0,'No capturado',d18.ejec18) AS 'ejecutivo' " &_
			"FROM " & strOficina & "_extranet." & tablamov & " AS i " &_
			"LEFT JOIN " & strOficina & "_extranet.c01refer AS c ON i.refcia01 = c.refe01 " &_
