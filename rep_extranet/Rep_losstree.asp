	...
	Else
		...
		Do Until RSops.EOF
				...
				datos = datos &	"<tr>" &_
								...
								celdadatos(RSops.Fields.Item("monto").Value)
				datos = datos &	"</tr>"
...
'					" if(cta.cgas31 is null and bs.fechst01 is null and etx.id_resp is null, 'UNILEVER',( IF(etx.id_resp is not null or etx.id_resp <> '' or etx.id_resp <> 0,   (select group_concat(distinct cres.nom_resp) from rku_status.cat_resp as cres where cres.id_resp = etx.id_resp) , 'GRUPO ZEGO') )) as 'Responsable', " & _
	...
	For i = 0 to 5
		...
		Select Case i
				Case 0
					aduanaTmp = "rku"
					adu= "'10110825 - GRUPO REYES KURI,  S.C.'"
				Case 1
					aduanaTmp = "dai"
					adu = "'10110818 - DESPACHOS AEREOS INTEGRADOS, S.C.'"
				Case 2
					aduanaTmp = "tol"
					adu = "'10110819 - COMERCIO EXTERIOR DEL GOLFO, S.C.'"
				Case 3
					aduanaTmp = "sap"
					adu = "'10080746 - SERVADUANALES DEL PACIFICO, S.C.'"
				Case 4
					aduanaTmp = "lzr"
					adu = "'10080746 - SERVADUANALES DEL PACIFICO, S.C.'"
				Case 5
		...
		SQL = SQL & "SELECT 'IMPORTACION' as 'Tipo de Operacion', " & chr(13) & chr(10)
			...
			SQL = SQL & "ifnull(DATE_FORMAT(c.fdsp01,'%d-%b-%y'),DATE_FORMAT(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),'%d-%b-%y')) AS 'Fecha de Despacho', " & chr(13) & chr(10)
			...
			SQL = SQL & "(select sum(if(d1.conc11 = 'ANT',d1.mont11,if(d1.conc11='CAN',d1.mont11 * (-1),NULL))) from "&aduanaTmp&"_extranet.d11movim as d1 where d1.refe11 = i.refcia01 and d1.conc11 in ('ANT','CAN')) as monto, " & chr(13) & chr(10)
		...
		SQL = SQL & "WHERE i.cveped01 <> 'R1' " & chr(13) & chr(10)
			'SQL = SQL & "and i.firmae01 is not null and i.firmae01 <> '' " & chr(13) & chr(10)
			'SQL = SQL & "and ((c.fdsp01 <> '0000-00-00' and c.fdsp01 <> '' and c.fdsp01 IS NOT NULL) or (bs.Fechst01 <> '00000000' and bs.Fechst01 <> '' and bs.Fechst01 IS NOT NULL) ) " & chr(13) & chr(10)
		...
		SQL = SQL & "HAVING (esta31 is null or esta31 = 'I') and (frec31 is null or frec31 = '0000-00-00') and (monto is null or monto <> 0 ) " & chr(13) & chr(10)

		if (i<>5) then
			SQL = SQL & "UNION ALL " & chr(13) & chr(10)
		else
			SQL = SQL & "ORDER BY 20 DESC "
		end if
		 ' Response.Write(SQL)
	 ' Response.End
	Next
...
<HTML>
	<HEAD>
		<TITLE>::.... REPORTE ÁRBOL DE PERDIDAS .... ::</TITLE>
	</HEAD>
	<BODY>
		<%=html%>
	</BODY>
