	...
	Else
		...
		Do Until RSops.EOF
				...
				datos = datos &	"<tr>" &_
								...
								celdadatos(RSops.Fields.Item("Fecha Solucion").Value)
				datos = datos &	"</tr>"
...
function GeneraSQL
	...
	SQL = 	"SELECT 'IMPORTACION' as 'Tipo de Operacion', " &_
					"if(d18.ejec18 is not null,d18.ejec18,'') AS 'Key Account', " &_
					...
					"IF(cau.c01causa is not null and cau.c01causa <> '', cau.c01causa,'EN RECOLECCIÓN DE DATOS') as 'Causal', " &_
					"IF(etx.id_resp is not null and etx.id_resp <> '' and etx.id_resp <> 0,(select cres.nom_resp from rku_status.cat_resp as cres where cres.id_resp = etx.id_resp),'GRUPO ZEGO') as 'Responsable', " &_
					"ABS(IF( CURDATE() >= DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY) , DATEDIFF(CURDATE(),DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY)) , DATEDIFF(DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY),CURDATE()))) as 'Dias Transcurridos', " &_
					"IF(etx.f_solucion is not null and etx.f_solucion <> '' and etx.f_solucion <> '0000-00-00',DATE_FORMAT(etx.f_solucion,'%d-%b-%y'),'No Capturada') as 'Fecha Solucion' " &_
			...
			"WHERE i.cveped01 <> 'R1' " &_
					"AND i.firmae01 IS NOT NULL AND i.firmae01 <> '' " &_
					"AND i.rfccli01 in ('UME651115N48','BRM711115GI8','ISI011214HM3') " &_
					"and (i.fecpag01 >=  '" & DateI & "' AND i.fecpag01 <= '" & DateF & "') " &_
					"and (bs.Fechst01 is not null and bs.Fechst01 <> '') " &_
					"and ((cta.cgas31 IS NULL or cta.cgas31 = '0000000' or cta.cgas31 = '') OR ((cta.fech31 IS NULL OR cta.fech31 = '0000-00-00' OR cta.fech31 = ''))) " &_
			...
			"SELECT 'IMPORTACION' as 'Tipo de Operacion', " &_
					...
					"fr.d_mer102 AS 'Descripcion de la Mercancia', " &_
					"IF(cau.c01causa is not null and cau.c01causa <> '', cau.c01causa,'EN RECOLECCIÓN DE DATOS') as 'Causal', " &_
					"IF(etx.id_resp is not null and etx.id_resp <> '' and etx.id_resp <> 0,(select cres.nom_resp from dai_status.cat_resp as cres where cres.id_resp = etx.id_resp),'GRUPO ZEGO') as 'Responsable', " &_
					"ABS(IF( CURDATE() >= DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY) , DATEDIFF(CURDATE(),DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY)) , DATEDIFF(DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY),CURDATE()))) as 'Dias Transcurridos', " &_
					"IF(etx.f_solucion is not null and etx.f_solucion <> '' and etx.f_solucion <> '0000-00-00',DATE_FORMAT(etx.f_solucion,'%d-%b-%y'),'No Capturada') as 'Fecha Solucion' " &_
			...
			"SELECT 'IMPORTACION' as 'Tipo de Operacion', " &_
					...
					"fr.d_mer102 AS 'Descripcion de la Mercancia', " &_
					"IF(cau.c01causa is not null and cau.c01causa <> '', cau.c01causa,'EN RECOLECCIÓN DE DATOS') as 'Causal', " &_
					"IF(etx.id_resp is not null and etx.id_resp <> '' and etx.id_resp <> 0,(select cres.nom_resp from sap_status.cat_resp as cres where cres.id_resp = etx.id_resp),'GRUPO ZEGO') as 'Responsable', " &_
					"ABS(IF( CURDATE() >= DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY) , DATEDIFF(CURDATE(),DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY)) , DATEDIFF(DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY),CURDATE()))) as 'Dias Transcurridos', " &_
					"IF(etx.f_solucion is not null and etx.f_solucion <> '' and etx.f_solucion <> '0000-00-00',DATE_FORMAT(etx.f_solucion,'%d-%b-%y'),'No Capturada') as 'Fecha Solucion' " &_
			...
			"GROUP BY i.refcia01, cau.c01causa " &_
			"UNION ALL " &_
			"SELECT 'EXPORTACION' as 'Tipo de Operacion', " &_
					"if(d18.ejec18 is not null,d18.ejec18,'') AS 'Key Account', " &_
					"DATE_FORMAT(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),'%d-%b-%y') AS 'Fecha de Despacho', " &_
					"i.refcia01 AS 'Referencia', " &_
					"DATE_FORMAT(i.fecpag01, '%y') as anopdto, " &_
			...
			"WHERE i.cveped01 <> 'R1' " &_
					...
					"and (bs.Fechst01 is not null and bs.Fechst01 <> '') " &_
					"and ((cta.cgas31 IS NULL or cta.cgas31 = '0000000' or cta.cgas31 = '') OR ((cta.fech31 IS NULL OR cta.fech31 = '0000-00-00' OR cta.fech31 = ''))) " &_
					"and (cta.anti31 > 0 OR cta.anti31 <> '' OR cta.anti31 IS NOT NULL) " &_
			"GROUP BY i.refcia01, cau.c01causa " &_
			...
			"SELECT 'IMPORTACION' as 'Tipo de Operacion', " &_
					...
					"fr.d_mer102 AS 'Descripcion de la Mercancia', " &_
					"IF(cau.c01causa is not null and cau.c01causa <> '', cau.c01causa,'EN RECOLECCIÓN DE DATOS') as 'Causal', " &_
					"IF(etx.id_resp is not null and etx.id_resp <> '' and etx.id_resp <> 0,(select cres.nom_resp from ceg_status.cat_resp as cres where cres.id_resp = etx.id_resp),'GRUPO ZEGO') as 'Responsable', " &_
					"ABS(IF( CURDATE() >= DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY) , DATEDIFF(CURDATE(),DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY)) , DATEDIFF(DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY),CURDATE()))) as 'Dias Transcurridos', " &_
					"IF(etx.f_solucion is not null and etx.f_solucion <> '' and etx.f_solucion <> '0000-00-00',DATE_FORMAT(etx.f_solucion,'%d-%b-%y'),'No Capturada') as 'Fecha Solucion' " &_
			...
			"SELECT 'IMPORTACION' as 'Tipo de Operacion', " &_
					...
					"fr.d_mer102 AS 'Descripcion de la Mercancia', " &_
					"IF(cau.c01causa is not null and cau.c01causa <> '', cau.c01causa,'EN RECOLECCIÓN DE DATOS') as 'Causal', " &_
					"IF(etx.id_resp is not null and etx.id_resp <> '' and etx.id_resp <> 0,(select cres.nom_resp from lzr_status.cat_resp as cres where cres.id_resp = etx.id_resp),'GRUPO ZEGO') as 'Responsable', " &_
					"ABS(IF( CURDATE() >= DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY) , DATEDIFF(CURDATE(),DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY)) , DATEDIFF(DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY),CURDATE()))) as 'Dias Transcurridos', " &_
					"IF(etx.f_solucion is not null and etx.f_solucion <> '' and etx.f_solucion <> '0000-00-00',DATE_FORMAT(etx.f_solucion,'%d-%b-%y'),'No Capturada') as 'Fecha Solucion' " &_
			...
			"SELECT 'IMPORTACION' as 'Tipo de Operacion', " &_
					...
					"fr.d_mer102 AS 'Descripcion de la Mercancia', " &_
					"IF(cau.c01causa is not null and cau.c01causa <> '', cau.c01causa,'EN RECOLECCIÓN DE DATOS') as 'Causal', " &_
					"IF(etx.id_resp is not null and etx.id_resp <> '' and etx.id_resp <> 0,(select cres.nom_resp from tol_status.cat_resp as cres where cres.id_resp = etx.id_resp),'GRUPO ZEGO') as 'Responsable', " &_
					"ABS(IF( CURDATE() >= DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY) , DATEDIFF(CURDATE(),DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY)) , DATEDIFF(DATE_ADD(MAX(STR_TO_DATE(bs.Fechst01,'%d%m%Y')),INTERVAL 5 DAY),CURDATE()))) as 'Dias Transcurridos', " &_
					"IF(etx.f_solucion is not null and etx.f_solucion <> '' and etx.f_solucion <> '0000-00-00',DATE_FORMAT(etx.f_solucion,'%d-%b-%y'),'No Capturada') as 'Fecha Solucion' " &_
