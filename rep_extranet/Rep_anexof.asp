...
if  Session("GAduana") = "" then
	...
	'Response.Write("cantidad: " & RSops.RecordCount)
	...
	Else
		...
		header = 			"<tr class = ""boton"">" &_
								...
								celdahead("Entrada de Barco") &_
								celdahead("Fechas Facturas") &_
								celdahead("Candado Fiscal") &_
								celdahead("Destinos")
		...
		importe = 0
		Do Until RSops.EOF
			if RSops.Fields.Item("Sec Fraccion").Value = "1" then
				datos = datos &	"<tr>" &_
								...
								celdadatos(RSops.Fields.Item("Entrada de Barco").Value) &_
								celdadatos(RSops.Fields.Item("Fechas Facturas").Value) &_
								celdadatos(RSops.Fields.Item("Candado Fiscal").Value) &_
								celdadatos(RSops.Fields.Item("Destinos").Value)
...
function filtro
	if Vckcve = 0 then
		if Vrfc <> "0" then
			condicion = "AND ops.rfccli01 = '" & Vrfc & "' "
		else
			condicion = "AND ops.rfccli01 = 'CME8909276S1' "
		end if
	else
		if Vclave <> "Todos" Then
			condicion = "AND ops.cvecli01 = " & Vclave & " "
		Else
			permi = PermisoClientes(Session("GAduana"),strPermisos,"cvecli01")
			condicion = permi
			' Modificacion realizada por Manuel Alejandro Estevez Fernandez
			' 
...
function GeneraSQL
	...
	SQL = 	"SELECT ops.refcia01, " &_
			"CASE ops.cvecli01 " &_
				"WHEN '4284' THEN '5632' " &_
				"WHEN '4581' THEN '5654' " &_
				"WHEN '4582' THEN '5637' " &_
				"WHEN '4583' THEN '5634' " &_
				"WHEN '5152' THEN '5632' " &_
				"WHEN '5153' THEN '5654' " &_
				"WHEN '5154' THEN '5637' " &_
				"WHEN '5155' THEN '5634' " &_
				"ELSE '-------' " &_
			"END AS 'BU', " &_
			...
			"	  left join rku_extranet.d05artic AS d05  on d05.refe05 = rec.reforg06 " &_
			"WHERE rec.refcia06 = ops.refcia01 AND d05.agru05 = fra.ordfra02 AND d05.frac05 = fra.fraarn02   " &_
			"GROUP BY d05.refe05), " &_
			"(SELECT GROUP_CONCAT(DISTINCT(d05.pedi05))   " &_
			...
			"IF(ops.cveped01 = 'R1',  " &_
			"(SELECT GROUP_CONCAT(DISTINCT(d05.fact05) SEPARATOR ' | ')   " &_
			"FROM rku_extranet.ssrecp06 as rec " &_
			"	  left join rku_extranet.d05artic AS d05  on d05.refe05 = rec.reforg06   " &_
			"WHERE rec.refcia06 = ops.refcia01 AND d05.agru05 = fra.ordfra02 AND d05.frac05 = fra.fraarn02   " &_
			"GROUP BY d05.refe05), " &_
			...
			"WHERE rec.refcia06 = ops.refcia01 AND d05.agru05 = fra.ordfra02 AND d05.frac05 = fra.fraarn02 AND fac.patent39 = ops.patent01   " &_
			"GROUP BY d05.refe05), " &_
			"(SELECT GROUP_CONCAT(DISTINCT(fac.nompro39))   " &_
			"FROM rku_extranet.d05artic AS d05   " &_
			"LEFT JOIN rku_extranet.ssfact39 AS fac ON fac.refcia39 = d05.refe05 AND fac.numfac39 = d05.fact05   " &_
			...
			"ROUND(fra.vmerme02 * ops.factmo01, 2) AS 'Valor Factura USD', " &_
			"ops.tipcam01 AS 'Tipo de Cambio', " &_
			"ROUND(fra.vmerme02 * ops.factmo01 * ops.tipcam01, 2) AS 'Valor MN', " &_
			"ROUND(fra.vaduan02, 0) AS 'Valor Aduana', " &_
			"ops.pesobr01 AS 'Peso Bruto', " &_
			"fra.p_adv102 AS 'P_Advalorem', " &_
			"fra.i_adv102 AS 'Advalorem', " &_
			"fra.i_iva102 AS 'IVA', " &_
			"IFNULL(fra.dtafpp02, 0) AS 'DTA', " &_
			...
			"GROUP BY d21.refe21), 0), 0) AS 'Maniobras', " &_
			"IF(ops.cveped01 = 'R1', " &_
			"IFNULL((SELECT SUM(prv.import33)  " &_
			"FROM rku_extranet.sscont33 AS prv  " &_
			"WHERE prv.refcia33 = ops.refcia01 AND prv.patent33 = ops.patent01 AND prv.adusec33 = ops.adusec01 AND prv.cveimp33 = 15  " &_
			"GROUP BY prv.refcia33), 0), " &_
			...
			"IF(ops.cveped01 = 'R1', " &_
			"IFNULL((SELECT SUM(eci.import33)  " &_
			"FROM rku_extranet.sscont33 AS eci  " &_
			"WHERE eci.refcia33 = ops.refcia01 AND eci.patent33 = ops.patent01 AND eci.adusec33 = ops.adusec01 AND eci.cveimp33 = 18  " &_
			"GROUP BY eci.refcia33), 0), " &_
			...
			"LEFT JOIN rku_extranet.c21paghe AS c21 ON c21.clav21 = e21.conc21 " &_
			"WHERE d21.refe21 = ops.refcia01 AND c21.desc21 LIKE '%MANIOBR%' " &_
			"GROUP BY d21.refe21), 0), 0) AS 'Maniobras Puerto', " &_
			...
			"LEFT JOIN rku_extranet.e21paghe AS e21 ON e21.foli21 = d21.foli21 AND YEAR(e21.fech21) = YEAR(d21.fech21) AND e21.tmov21 = d21.tmov21 " &_
			"LEFT JOIN rku_extranet.c21paghe AS c21 ON c21.clav21 = e21.conc21 " &_
			"WHERE d21.refe21 = ops.refcia01 AND c21.desc21 LIKE '%RECON%ADUA%' " &_
			...
			"FROM rku_extranet.d21paghe AS d21 " &_
			"LEFT JOIN rku_extranet.e21paghe AS e21 ON e21.foli21=d21.foli21 AND YEAR(e21.fech21)=YEAR(d21.fech21) AND e21.tmov21=d21.tmov21 AND e21.esta21<>'S' " &_
			"LEFT JOIN rku_extranet.c21paghe AS c21 ON c21.clav21 = e21.conc21 " &_
			...
			"DATE_FORMAT(ops.fecent01, '%d/%m/%Y') AS 'Entrada de Barco', " &_
			"IF( ops.cveped01 = 'R1', " &_
			"(SELECT GROUP_CONCAT(DISTINCT(DATE_FORMAT(d05.ffactp05, '%d/%m/%Y')) SEPARATOR ', ')   " &_
			"FROM rku_extranet.ssrecp06 as rec " &_
			"	  LEFT JOIN rku_extranet.d05artic AS d05  on d05.refe05 = rec.reforg06  " &_
			"	  LEFT JOIN rku_extranet.ssfact39 AS fac ON fac.refcia39 = d05.refe05 AND fac.numfac39 = d05.fact05   " &_
			"WHERE rec.refcia06 = ops.refcia01 AND d05.agru05 = fra.ordfra02 AND d05.frac05 = fra.fraarn02 AND fac.patent39 = ops.patent01   " &_
			...
			"FROM rku_extranet.d05artic AS d05   " &_
			"WHERE d05.refe05 = ops.refcia01 AND d05.agru05 = fra.ordfra02 AND d05.frac05 = fra.fraarn02   " &_
			"GROUP BY d05.refe05)) AS 'Fechas Facturas', " &_
			"IFNULL((SELECT GROUP_CONCAT(DISTINCT(con.numcan40) SEPARATOR ', ') " &_
			"FROM rku_extranet.sscont40 AS con " &_
			"WHERE con.refcia40 = ops.refcia01 AND con.patent40 = ops.patent01 AND con.adusec40 = ops.adusec01 " &_
			"GROUP BY con.refcia40), '') AS 'Candado Fiscal', " &_
			"IFNULL((SELECT GROUP_CONCAT(DISTINCT(destino) SEPARATOR ' | ') " &_
			"FROM rku_cpsimples.tcepartidas AS t " &_
			"WHERE MID(t.Refe01, 1, 11) = MID(ops.refcia01, 1, 11) AND t.Frac01 = fra.fraarn02 " &_
			"GROUP BY MID(t.Refe01, 1, 11)), '') AS 'Destinos' " &_
			...
			"AND ((ops.fecpag01 >= '" & DateI & "' AND ops.fecpag01 <= '" & DateF & "')  OR (rec.f_pagr06 >= '" & DateI & "' AND rec.f_pagr06 <= '" & DateF & "')) " &_
			condicion &_
						
			"UNION ALL " &_
			
			"SELECT ops.refcia01, " &_
			"CASE ops.cvecli01 " &_
				"WHEN '2494' THEN '5654' " &_
				"ELSE '-------' " &_
			"END AS 'BU', " &_
			"con.agente32 AS 'Agente Aduanal', " &_
			"ops.nomadu01 AS 'Nombre Aduana', " &_
			...
			"IF(ops.cveped01 = 'R1', " &_
			"IFNULL((SELECT SUM(iva.import36) " &_
			"FROM sap_extranet.sscont36 AS iva " &_
			"WHERE iva.refcia36 = rec.reforg06 AND iva.cveimp36 = 3 " &_
			"GROUP BY iva.refcia36 " &_
			"), 0) " &_
			", 0) AS 'IVA del Pedimento Original', " &_
			"IF(ops.cveped01 = 'R1', " &_
			"IFNULL((SELECT SUM(adv.import36) " &_
			"FROM sap_extranet.sscont36 AS adv " &_
			"WHERE adv.refcia36 = rec.reforg06 AND adv.cveimp36 = 6 " &_
			"GROUP BY adv.refcia36 " &_
			"), 0) " &_
			", 0) AS 'IGI del Pedimento Original', " &_
			...
			"	  left join sap_extranet.d05artic AS d05  on d05.refe05 = rec.reforg06 " &_
			"WHERE rec.refcia06 = ops.refcia01 AND d05.agru05 = fra.ordfra02 AND d05.frac05 = fra.fraarn02   " &_
			"GROUP BY d05.refe05), " &_
			"(SELECT GROUP_CONCAT(DISTINCT(d05.pedi05))   " &_
			"FROM sap_extranet.d05artic AS d05   " &_
			"WHERE d05.refe05 = ops.refcia01 AND d05.agru05 = fra.ordfra02 AND d05.frac05 = fra.fraarn02   " &_
			"GROUP BY d05.refe05)) AS 'Referencia / Pedido',   " &_
			"IF(ops.cveped01 = 'R1',  " &_
			"(SELECT GROUP_CONCAT(DISTINCT(d05.fact05) SEPARATOR ' | ')   " &_
			...
			"FROM sap_extranet.ssrecp06 as rec " &_
			"	  LEFT JOIN sap_extranet.d05artic AS d05  on d05.refe05 = rec.reforg06  " &_
			"	  LEFT JOIN sap_extranet.ssfact39 AS fac ON fac.refcia39 = d05.refe05 AND fac.numfac39 = d05.fact05   " &_
			"WHERE rec.refcia06 = ops.refcia01 AND d05.agru05 = fra.ordfra02 AND d05.frac05 = fra.fraarn02 AND fac.patent39 = ops.patent01   " &_
			"GROUP BY d05.refe05), " &_
			"(SELECT GROUP_CONCAT(DISTINCT(fac.nompro39))   " &_
			"FROM sap_extranet.d05artic AS d05   " &_
			...
			"IFNULL(fra.dtafpp02, 0) AS 'DTA', " &_
			"IF(MID(ops.refcia01, 1, 3) IN ('TOL', 'DAI'), IFNULL((SELECT SUM(d21.mont21 * IF(e21.deha21 = 'C', -1, 1)) " &_
			"FROM sap_extranet.d21paghe AS d21 " &_
			"LEFT JOIN sap_extranet.e21paghe AS e21 ON e21.foli21 = d21.foli21 AND YEAR(e21.fech21) = YEAR(d21.fech21) AND e21.tmov21 = d21.tmov21 " &_
			"LEFT JOIN sap_extranet.c21paghe AS c21 ON c21.clav21 = e21.conc21 " &_
			"WHERE d21.refe21 = ops.refcia01 AND c21.desc21 LIKE '%MANIOBR%' " &_
			"GROUP BY d21.refe21), 0), 0) AS 'Maniobras', " &_
			"IF(ops.cveped01 = 'R1', " &_
			"IFNULL((SELECT SUM(prv.import33)  " &_
			"FROM sap_extranet.sscont33 AS prv  " &_
			"WHERE prv.refcia33 = ops.refcia01 AND prv.patent33 = ops.patent01 AND prv.adusec33 = ops.adusec01 AND prv.cveimp33 = 15  " &_
			"GROUP BY prv.refcia33), 0), " &_
			"IFNULL((SELECT SUM(prv.import36)  " &_
			"FROM sap_extranet.sscont36 AS prv  " &_
			"WHERE prv.refcia36 = ops.refcia01 AND prv.patent36 = ops.patent01 AND prv.adusec36 = ops.adusec01 AND prv.cveimp36 = 15  " &_
			"GROUP BY prv.refcia36), 0)) AS 'PRV', " &_
			"170 AS 'Validacion', " &_
			"IFNULL((SELECT SUM(e31.csce31) " &_
			"FROM sap_extranet.d31refer AS d31 " &_
			"INNER JOIN sap_extranet.e31cgast AS e31 ON e31.cgas31 = d31.cgas31 AND e31.esta31 <> 'C' " &_
			"WHERE d31.refe31 = ops.refcia01), 0) AS 'Serv_Complementarios', " &_
			"IFNULL((SELECT SUM(d21.mont21 * IF(e21.deha21 = 'C', -1, 1)) " &_
			"FROM sap_extranet.d21paghe AS d21 " &_
			"LEFT JOIN sap_extranet.e21paghe AS e21 ON e21.foli21 = d21.foli21 AND YEAR(e21.fech21) = YEAR(d21.fech21) AND e21.tmov21 = d21.tmov21 " &_
			...
			"IF(ops.cveped01 = 'R1', " &_
			"IFNULL((SELECT SUM(eci.import33)  " &_
			"FROM sap_extranet.sscont33 AS eci  " &_
			"WHERE eci.refcia33 = ops.refcia01 AND eci.patent33 = ops.patent01 AND eci.adusec33 = ops.adusec01 AND eci.cveimp33 = 18  " &_
			"GROUP BY eci.refcia33), 0), " &_
			"IFNULL((SELECT SUM(eci.import36)  " &_
			"FROM sap_extranet.sscont36 AS eci  " &_
			"WHERE eci.refcia36 = ops.refcia01 AND eci.patent36 = ops.patent01 AND eci.adusec36 = ops.adusec01 AND eci.cveimp36 = 18  " &_
			"GROUP BY eci.refcia36), 0)) AS 'ECI', " &_
			"0 AS 'Otros Pagos', " &_
			"IF(MID(ops.refcia01, 1, 3) NOT IN ('TOL', 'DAI'), IFNULL((SELECT SUM(d21.mont21 * IF(e21.deha21 = 'C', -1, 1)) " &_
			...
			"IFNULL((SELECT GROUP_CONCAT(c20.nomb20) " &_
			"FROM sap_extranet.d21paghe AS d21 " &_
			"LEFT JOIN sap_extranet.e21paghe AS e21 ON e21.foli21=d21.foli21 AND YEAR(e21.fech21)=YEAR(d21.fech21) AND e21.tmov21=d21.tmov21 AND e21.esta21<>'S' " &_
			"LEFT JOIN sap_extranet.c21paghe AS c21 ON c21.clav21 = e21.conc21 " &_
			"LEFT JOIN sap_extranet.c20benef AS c20 ON c20.clav20 = e21.bene21 AND c20.aplic20 <> 'T' " &_
			"WHERE d21.refe21 = ops.refcia01 AND c21.desc21 LIKE '%FLETE%' " &_	
			"GROUP BY e21.conc21), '') AS 'transportista', " &_
			"'' AS 'Flete Terrestre', " &_
			"'' AS 'Naviera', " &_
			...
			"fra.paiori02 AS 'Pais Origen', " &_
			"IFNULL((SELECT DISTINCT IF(par.cveide12 IS NOT NULL AND par.cveide12 <> '', 'Si', 'No') " &_
			"FROM sap_extranet.ssipar12 AS par " &_
			"WHERE par.refcia12 = ops.refcia01 AND par.patent12 = ops.patent01 AND par.adusec12 = ops.adusec01 AND par.ordfra12 = fra.ordfra02 " &_
			"AND par.cveide12 IN ('AL', 'CM', 'CP', 'EF', 'PS', 'TL')), 'No') AS 'Beneficio Arancelario', " &_
			"(SELECT GROUP_CONCAT(DISTINCT(fac.terfac39)) " &_
			"FROM sap_extranet.ssfact39 AS fac " &_
			"WHERE fac.refcia39 = ops.refcia01 AND fac.patent39 = ops.patent01 AND fac.adusec39 = ops.adusec01 " &_
			"GROUP BY fac.refcia39) AS 'Incoterm', " &_
			"fra.vincul02 AS 'Vinculacion', " &_
			"fra.metval02 AS 'Valoracion', " &_
			...
			"ops.decble01 AS 'Decrementables', " &_
			"IFNULL((SELECT DISTINCT IF(rc.refcia06 IS NOT NULL AND rc.refcia06 <> '', 'Si', 'No') " &_
			"FROM sap_extranet.ssrecp06 AS rc " &_
			"WHERE rc.reforg06 = ops.refcia01), 'No') AS 'Rectificacion', " &_
			"'' AS 'Fecha', " &_
			"'' AS 'NUM_POLIZA', " &_
			"'' AS 'TIPO_POLIZA', " &_
			...
			"'' AS 'CARPETA', " &_
			"IFNULL((SELECT GROUP_CONCAT(DISTINCT(con.numcon40) SEPARATOR ', ') " &_
			"FROM sap_extranet.sscont40 AS con " &_
			"WHERE con.refcia40 = ops.refcia01 AND con.patent40 = ops.patent01 AND con.adusec40 = ops.adusec01 " &_
			"GROUP BY con.refcia40), '') AS 'Contenedores', " &_
			"DATE_FORMAT(ops.fecent01, '%d/%m/%Y') AS 'Entrada de Barco', " &_
			"IF( ops.cveped01 = 'R1', " &_
			...
			"WHERE MID(t.Refe01, 1, 11) = MID(ops.refcia01, 1, 11) AND t.Frac01 = fra.fraarn02 " &_
			"GROUP BY MID(t.Refe01, 1, 11)), '') AS 'Destinos' " &_
			"FROM sap_extranet.ssdagi01 AS ops " &_
			"INNER JOIN sap_extranet.ssconf32 AS con ON con.patent32 = ops.patent01 AND con.cveadu32 = ops.cveadu01 AND con.cvesec32 = ops.cvesec01 " &_
			"LEFT JOIN sap_extranet.ssrecp06 AS rec ON rec.refcia06 = ops.refcia01 AND rec.patent06 = ops.patent01 " &_
			"AND rec.adusec06 = ops.adusec01 " &_
			"LEFT JOIN sap_extranet.ssfrac02 AS fra ON fra.refcia02 = ops.refcia01 AND fra.patent02 = ops.patent01 AND fra.adusec02 = ops.adusec01 " &_
			"WHERE ops.firmae01 <> '' AND ops.firmae01 IS NOT NULL " &_
			"AND ((ops.fecpag01 >= '" & DateI & "' AND ops.fecpag01 <= '" & DateF & "')  OR (rec.f_pagr06 >= '" & DateI & "' AND rec.f_pagr06 <= '" & DateF & "')) " &_
			condicion &_
			...
			"IF(ops.cveped01 = 'R1', " &_
			"IFNULL((SELECT SUM(iva.import36) " &_
			"FROM dai_extranet.sscont36 AS iva " &_
			"WHERE iva.refcia36 = rec.reforg06 AND iva.cveimp36 = 3 " &_
			"GROUP BY iva.refcia36 " &_
			"), 0) " &_
			", 0) AS 'IVA del Pedimento Original', " &_
			"IF(ops.cveped01 = 'R1', " &_
			"IFNULL((SELECT SUM(adv.import36) " &_
			"FROM dai_extranet.sscont36 AS adv " &_
			"WHERE adv.refcia36 = rec.reforg06 AND adv.cveimp36 = 6 " &_
			"GROUP BY adv.refcia36 " &_
			"), 0) " &_
			", 0) AS 'IGI del Pedimento Original', " &_
			...
			"	  left join dai_extranet.d05artic AS d05  on d05.refe05 = rec.reforg06 " &_
			"WHERE rec.refcia06 = ops.refcia01 AND d05.agru05 = fra.ordfra02 AND d05.frac05 = fra.fraarn02   " &_
			"GROUP BY d05.refe05), " &_
			"(SELECT GROUP_CONCAT(DISTINCT(d05.pedi05))   " &_
			"FROM dai_extranet.d05artic AS d05   " &_
			"WHERE d05.refe05 = ops.refcia01 AND d05.agru05 = fra.ordfra02 AND d05.frac05 = fra.fraarn02   " &_
			"GROUP BY d05.refe05)) AS 'Referencia / Pedido',   " &_
			"IF(ops.cveped01 = 'R1',  " &_
			"(SELECT GROUP_CONCAT(DISTINCT(d05.fact05) SEPARATOR ' | ')   " &_
			...
			"FROM dai_extranet.ssrecp06 as rec " &_
			"	  LEFT JOIN dai_extranet.d05artic AS d05  on d05.refe05 = rec.reforg06  " &_
			"	  LEFT JOIN dai_extranet.ssfact39 AS fac ON fac.refcia39 = d05.refe05 AND fac.numfac39 = d05.fact05   " &_
			"WHERE rec.refcia06 = ops.refcia01 AND d05.agru05 = fra.ordfra02 AND d05.frac05 = fra.fraarn02 AND fac.patent39 = ops.patent01   " &_
			"GROUP BY d05.refe05), " &_
			"(SELECT GROUP_CONCAT(DISTINCT(fac.nompro39))   " &_
			"FROM dai_extranet.d05artic AS d05   " &_
			...
			"IFNULL(fra.dtafpp02, 0) AS 'DTA', " &_
			"IF(MID(ops.refcia01, 1, 3) IN ('TOL', 'DAI'), IFNULL((SELECT SUM(d21.mont21 * IF(e21.deha21 = 'C', -1, 1)) " &_
			"FROM dai_extranet.d21paghe AS d21 " &_
			"LEFT JOIN dai_extranet.e21paghe AS e21 ON e21.foli21 = d21.foli21 AND YEAR(e21.fech21) = YEAR(d21.fech21) AND e21.tmov21 = d21.tmov21 " &_
			"LEFT JOIN dai_extranet.c21paghe AS c21 ON c21.clav21 = e21.conc21 " &_
			"WHERE d21.refe21 = ops.refcia01 AND c21.desc21 LIKE '%MANIOBR%' " &_
			"GROUP BY d21.refe21), 0), 0) AS 'Maniobras', " &_
			"IF(ops.cveped01 = 'R1', " &_
			"IFNULL((SELECT SUM(prv.import33)  " &_
			"FROM dai_extranet.sscont33 AS prv  " &_
			"WHERE prv.refcia33 = ops.refcia01 AND prv.patent33 = ops.patent01 AND prv.adusec33 = ops.adusec01 AND prv.cveimp33 = 15  " &_
			"GROUP BY prv.refcia33), 0), " &_
			"IFNULL((SELECT SUM(prv.import36)  " &_
			"FROM dai_extranet.sscont36 AS prv  " &_
			"WHERE prv.refcia36 = ops.refcia01 AND prv.patent36 = ops.patent01 AND prv.adusec36 = ops.adusec01 AND prv.cveimp36 = 15  " &_
			"GROUP BY prv.refcia36), 0)) AS 'PRV', " &_
			"40 AS 'Validacion', " &_
			"IFNULL((SELECT SUM(e31.csce31) " &_
			"FROM dai_extranet.d31refer AS d31 " &_
			"INNER JOIN dai_extranet.e31cgast AS e31 ON e31.cgas31 = d31.cgas31 AND e31.esta31 <> 'C' " &_
			"WHERE d31.refe31 = ops.refcia01), 0) AS 'Serv_Complementarios', " &_
			"IFNULL((SELECT SUM(d21.mont21 * IF(e21.deha21 = 'C', -1, 1)) " &_
			"FROM dai_extranet.d21paghe AS d21 " &_
			"LEFT JOIN dai_extranet.e21paghe AS e21 ON e21.foli21 = d21.foli21 AND YEAR(e21.fech21) = YEAR(d21.fech21) AND e21.tmov21 = d21.tmov21 " &_
			...
			"IF(ops.cveped01 = 'R1', " &_
			"IFNULL((SELECT SUM(eci.import33)  " &_
			"FROM dai_extranet.sscont33 AS eci  " &_
			"WHERE eci.refcia33 = ops.refcia01 AND eci.patent33 = ops.patent01 AND eci.adusec33 = ops.adusec01 AND eci.cveimp33 = 18  " &_
			"GROUP BY eci.refcia33), 0), " &_
			"IFNULL((SELECT SUM(eci.import36)  " &_
			"FROM dai_extranet.sscont36 AS eci  " &_
			"WHERE eci.refcia36 = ops.refcia01 AND eci.patent36 = ops.patent01 AND eci.adusec36 = ops.adusec01 AND eci.cveimp36 = 18  " &_
			"GROUP BY eci.refcia36), 0)) AS 'ECI', " &_
			"0 AS 'Otros Pagos', " &_
			"IF(MID(ops.refcia01, 1, 3) NOT IN ('TOL', 'DAI'), IFNULL((SELECT SUM(d21.mont21 * IF(e21.deha21 = 'C', -1, 1)) " &_
			...
			"IFNULL((SELECT GROUP_CONCAT(c20.nomb20) " &_
			"FROM dai_extranet.d21paghe AS d21 " &_
			"LEFT JOIN dai_extranet.e21paghe AS e21 ON e21.foli21=d21.foli21 AND YEAR(e21.fech21)=YEAR(d21.fech21) AND e21.tmov21=d21.tmov21 AND e21.esta21<>'S' " &_
			"LEFT JOIN dai_extranet.c21paghe AS c21 ON c21.clav21 = e21.conc21 " &_
			"LEFT JOIN dai_extranet.c20benef AS c20 ON c20.clav20 = e21.bene21 AND c20.aplic20 <> 'T' " &_
			"WHERE d21.refe21 = ops.refcia01 AND c21.desc21 LIKE '%FLETE%' " &_	
			"GROUP BY e21.conc21), '') AS 'transportista', " &_
			"'' AS 'Flete Terrestre', " &_
			"'' AS 'Naviera', " &_
			...
			"fra.paiori02 AS 'Pais Origen', " &_
			"IFNULL((SELECT DISTINCT IF(par.cveide12 IS NOT NULL AND par.cveide12 <> '', 'Si', 'No') " &_
			"FROM dai_extranet.ssipar12 AS par " &_
			"WHERE par.refcia12 = ops.refcia01 AND par.patent12 = ops.patent01 AND par.adusec12 = ops.adusec01 AND par.ordfra12 = fra.ordfra02 " &_
			"AND par.cveide12 IN ('AL', 'CM', 'CP', 'EF', 'PS', 'TL')), 'No') AS 'Beneficio Arancelario', " &_
			"(SELECT GROUP_CONCAT(DISTINCT(fac.terfac39)) " &_
			"FROM dai_extranet.ssfact39 AS fac " &_
			"WHERE fac.refcia39 = ops.refcia01 AND fac.patent39 = ops.patent01 AND fac.adusec39 = ops.adusec01 " &_
			"GROUP BY fac.refcia39) AS 'Incoterm', " &_
			"fra.vincul02 AS 'Vinculacion', " &_
			"fra.metval02 AS 'Valoracion', " &_
			...
			"ops.decble01 AS 'Decrementables', " &_
			"IFNULL((SELECT DISTINCT IF(rc.refcia06 IS NOT NULL AND rc.refcia06 <> '', 'Si', 'No') " &_
			"FROM dai_extranet.ssrecp06 AS rc " &_
			"WHERE rc.reforg06 = ops.refcia01), 'No') AS 'Rectificacion', " &_
			"'' AS 'Fecha', " &_
			"'' AS 'NUM_POLIZA', " &_
			"'' AS 'TIPO_POLIZA', " &_
			...
			"'' AS 'CARPETA', " &_
			"IFNULL((SELECT GROUP_CONCAT(DISTINCT(con.numcon40) SEPARATOR ', ') " &_
			"FROM dai_extranet.sscont40 AS con " &_
			"WHERE con.refcia40 = ops.refcia01 AND con.patent40 = ops.patent01 AND con.adusec40 = ops.adusec01 " &_
			"GROUP BY con.refcia40), '') AS 'Contenedores', " &_
			"DATE_FORMAT(ops.fecent01, '%d/%m/%Y') AS 'Entrada de Barco', " &_
			"IF( ops.cveped01 = 'R1', " &_
			...
			"WHERE MID(t.Refe01, 1, 11) = MID(ops.refcia01, 1, 11) AND t.Frac01 = fra.fraarn02 " &_
			"GROUP BY MID(t.Refe01, 1, 11)), '') AS 'Destinos' " &_
			"FROM dai_extranet.ssdagi01 AS ops " &_
			"INNER JOIN dai_extranet.ssconf32 AS con ON con.patent32 = ops.patent01 AND con.cveadu32 = ops.cveadu01 AND con.cvesec32 = ops.cvesec01 " &_
			"LEFT JOIN dai_extranet.ssrecp06 AS rec ON rec.refcia06 = ops.refcia01 AND rec.patent06 = ops.patent01 " &_
			"AND rec.adusec06 = ops.adusec01 " &_
			"LEFT JOIN dai_extranet.ssfrac02 AS fra ON fra.refcia02 = ops.refcia01 AND fra.patent02 = ops.patent01 AND fra.adusec02 = ops.adusec01 " &_
			"WHERE ops.firmae01 <> '' AND ops.firmae01 IS NOT NULL " &_
			"AND ((ops.fecpag01 >= '" & DateI & "' AND ops.fecpag01 <= '" & DateF & "')  OR (rec.f_pagr06 >= '" & DateI & "' AND rec.f_pagr06 <= '" & DateF & "')) " &_
			condicion &_
			...
			"WHERE con.refcia40 = ops.refcia01 AND con.patent40 = ops.patent01 AND con.adusec40 = ops.adusec01 " &_
			"GROUP BY con.refcia40), '') AS 'Contenedores', " &_
			"DATE_FORMAT(ops.fecpre01, '%d/%m/%Y') AS 'Entrada de Barco', " &_
			"IF( ops.cveped01 = 'R1', " &_
			"(SELECT GROUP_CONCAT(DISTINCT(DATE_FORMAT(d05.ffactp05, '%d/%m/%Y')) SEPARATOR ', ')   " &_
			"FROM rku_extranet.ssrecp06 as rec " &_
			"	  LEFT JOIN rku_extranet.d05artic AS d05  on d05.refe05 = rec.reforg06  " &_
			...
			"WHERE MID(t.Refe01, 1, 11) = MID(ops.refcia01, 1, 11) AND t.Frac01 = fra.fraarn02 " &_
			"GROUP BY MID(t.Refe01, 1, 11)), '') AS 'Destinos' " &_
			"FROM rku_extranet.ssdage01 AS ops " &_
			"INNER JOIN rku_extranet.ssconf32 AS con ON con.patent32 = ops.patent01 AND con.cveadu32 = ops.cveadu01 AND con.cvesec32 = ops.cvesec01 " &_
			"LEFT JOIN rku_extranet.ssrecp06 AS rec ON rec.refcia06 = ops.refcia01 AND rec.patent06 = ops.patent01 " &_
			"AND rec.adusec06 = ops.adusec01 " &_
			"LEFT JOIN rku_extranet.ssfrac02 AS fra ON fra.refcia02 = ops.refcia01 AND fra.patent02 = ops.patent01 AND fra.adusec02 = ops.adusec01 " &_
			...
			"WHERE MID(t.Refe01, 1, 11) = MID(ops.refcia01, 1, 11) AND t.Frac01 = fra.fraarn02 " &_
			"GROUP BY MID(t.Refe01, 1, 11)), '') AS 'Destinos' " &_
			"FROM sap_extranet.ssdage01 AS ops " &_
			"INNER JOIN sap_extranet.ssconf32 AS con ON con.patent32 = ops.patent01 AND con.cveadu32 = ops.cveadu01 AND con.cvesec32 = ops.cvesec01 " &_
			"LEFT JOIN sap_extranet.ssrecp06 AS rec ON rec.refcia06 = ops.refcia01 AND rec.patent06 = ops.patent01 " &_
			"AND rec.adusec06 = ops.adusec01 " &_
			"LEFT JOIN sap_extranet.ssfrac02 AS fra ON fra.refcia02 = ops.refcia01 AND fra.patent02 = ops.patent01 AND fra.adusec02 = ops.adusec01 " &_
			...
			"WHERE MID(t.Refe01, 1, 11) = MID(ops.refcia01, 1, 11) AND t.Frac01 = fra.fraarn02 " &_
			"GROUP BY MID(t.Refe01, 1, 11)), '') AS 'Destinos' " &_
			"FROM dai_extranet.ssdage01 AS ops " &_
			"INNER JOIN dai_extranet.ssconf32 AS con ON con.patent32 = ops.patent01 AND con.cveadu32 = ops.cveadu01 AND con.cvesec32 = ops.cvesec01 " &_
			"LEFT JOIN dai_extranet.ssrecp06 AS rec ON rec.refcia06 = ops.refcia01 AND rec.patent06 = ops.patent01 " &_
			"AND rec.adusec06 = ops.adusec01 " &_
			"LEFT JOIN dai_extranet.ssfrac02 AS fra ON fra.refcia02 = ops.refcia01 AND fra.patent02 = ops.patent01 AND fra.adusec02 = ops.adusec01 " &_
