...
'http://10.66.1.9/PortalMySQL/Extranet/ext-Asp/Reportes/RepSegOpsGivaudan_Rep.asp'
...
if  Session("GAduana") = "" then
	...
	Else
		...
		End If
		if mov = "i" Then
			header = header &	celdahead("Operaciones con Preferencia Arancelaria") &_
								celdahead("Comentarios por aplicaci&oacute;n de Preferencia Arancelaria") &_
								celdahead("Operaciones con muestreo en Rojos") &_
								celdahead("Comentarios de Rectificaciones") 
		...
		Do Until RSops.EOF
			...
			if mov = "i" Then
				' if RSops.Fields.Item("rfccli01").Value="UME651115N48" or  RSops.Fields.Item("rfccli01").Value="BRM711115GI8" or  RSops.Fields.Item("rfccli01").Value="ISI011214HM3" or  RSops.Fields.Item("rfccli01").Value="UMA011214255" then  'se comento porque a flor le salian Especiales cuando eran CDU
					' clave =RSops.Fields.Item("cvecli")
					...
					' end if
					' 'OTRAS
					' if clave = "13000" or clave="14000" or clave="15005" or clve="11006" then
					' ubica = "Todas"
					' end if
			...
			datos = datos &	"<tr>" &_
								...
								celdadatos(RSops.Fields.Item("GUIABL").Value)
			if mov = "i" Then
			...
			Else
				...
				datos = datos & celdadatos(facturas)&_
						celdadatos(RSops.Fields.Item("TM").Value)
			End If
			...
			if mov = "i" Then
				...
				datos = datos &	celdadatos(Causales(Referencia, "T",RSops.Fields.Item("rfccli01").Value))
			else
			...
			datos = datos &		celdadatos(RSops.Fields.Item("FCG").Value) &_
								...
								celdadatos("&nbsp;")
			if mov = "i" Then
				datos = datos & celdadatos(Causales(Referencia, "A",RSops.Fields.Item("rfccli01").Value)) &_
			...
			if mov= "i" Then  
				datos = datos & celdadatos(RSops.Fields.Item("Preferencia").Value) &_
								celdadatos("&nbsp;") &_
								celdadatos(RSops.Fields.Item("ROJO").Value) &_
								celdadatos("&nbsp;")
...
function GeneraSQL
	...
	SQL = SQL & kpi("", "c.frev01", "c.fdsp01") & "AS 'KPIGRK', " &_
			...
			"(select group_concat(distinct p.cveide12) from " & strOficina & "_extranet.ssipar12 as p where p.refcia12=i.refcia01 and p.patent12=i.patent01 and p.adusec12=i.adusec01 and p.cveide12 in ('TL', 'AL', 'PS')) AS Preferencia," &_
			"(SELECT CASE(select count(cds.desdsc01)from trackingbahia.cat_det_situaciones AS cds where cds.detsit01 in (select bs.detsit01 from trackingbahia.bit_soia as bs where  bs.Numped01=i.numped01 and bs.Adusec01=i.adusec01 and bs.Numpat01=i.patent01 and bs.Cvesit01 in (select cs.cvesit01 from trackingbahia.cat_situaciones as cs )  ) and cds.desdsc01 in ('ROJO PS','ROJO SS')) WHEN 0 THEN '' WHEN 1 THEN 'SI' ELSE 'SI' END) AS ROJO " &_
