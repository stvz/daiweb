...
permitemp = PermisoClientesTabla("B","VER" ,strPermisos,"clie31")
...
if strTipoPedimento  = "2" then
   tmpTipo = "EXPORTACION"
   strSQL = "SELECT tipopr01, ifnull(factmo01,0) as factmo01, p_dta101, t_reca01, i_dta101, cvecli01, refcia01, fecpag01, ifnull(valfac01,0) as valfac01, fletes01, segros01, cvepvc01, tipcam01, patent01, numped01, totbul01, cveped01, cveadu01, desf0101, nompro01, cvepod01, nombar01, tipopr01, fecpre01 FROM ssdage01 WHERE fecpag01 >='"&FormatoFechaInv(strDateIni)&"' AND fecpag01 <='"&FormatoFechaInv(strDateFin)&"' " & Permi & " and firmae01 !='' order by refcia01"
  ...
  ' Comienza el HTML, se pintan los titulos de las columnas
   ...
   strCtaGas=""
	 ...
	 dblsald31=0
      ...
      strSQL1="select A.cgas31,B.fech31,B.suph31, B.coad31, B.csce31, B.chon31, B.piva31, B.anti31,B.sald31 from d31refer as A LEFT JOIN e31cgast as B ON A.cgas31=B.cgas31 where B.esta31='I' and A.refe31='"&strRefer&"' AND (" & permitemp & ")"
        Set RsRep1 = Server.CreateObject("ADODB.Recordset")

        IF Session("GAduana") <> "LAR" then
        CONN = ODBC_POR_ADUANA("VER")
        else
        CONN = ODBC_POR_ADUANA(Session("GAduana"))
        end if
        RsRep1.ActiveConnection =CONN
...
'Sus parametros son
      ...
      'Aqui se obtienen las fracciones por referencia
			strSQL2="select fraarn02,d_mer102,cancom02,cancom02,u_medc02,cantar02,u_medt02,ifnull(vmerme02,0) as vmerme02,vaduan02,ifnull(tasadv02,0) as tasadv02,ifnull(p_adv102,0) as p_adv102,ifnull(i_adv102,0) as i_adv102,ifnull(i_iva102,0) as i_iva102,ifnull(i_adv102,0) as i_adv102,ifnull(i_adv202,0) as i_adv202,i_cc0102,ifnull(i_cc0202,0) as i_cc0202 from ssfrac02 where refcia02='"&pRefer&"'"
