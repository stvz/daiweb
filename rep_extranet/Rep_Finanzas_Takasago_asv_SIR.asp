...
<%
' ######## R E P O R T E  A  M O D I F I C A R  P A R A  S I R 
'On Error Resume Next
	Dim FechaI, FechaF, Corte,Permiso,contador
	contador=5
	...
	html = "<br></br><div align=""center""><p  class=""Titulo1"">:: USUARIO NO HABILITADO PARA ACCESAR A LOS REPORTES ::</div></p></div>"
			...
			header = 			"<tr class = ""boton"">" 
				...
				if strOficina<>"sfi" and strOficina<>"Todas" then
					...
					Response.Write(info & header & datos)
				...
				elseif strOficina="Todas" then
					...
					datos=retornaMYSQL(mov,strOficina,FechaI,FechaF)
				
					 'SQL
					 datos=datos & datosSQL(mov,Vrfc,FechaI,FechaF)&"</table>"
					 response.write( info & header & datos)
				 ...
				 elseif strOficina="sfi" then
					...
					datos=datosSQL(mov,Vrfc,FechaI,FechaF)&"</table>"
					 Response.Write(info & header & datos)
					 Response.End()
					html = info & header & datos
				end if
...
function datosSQL(Toperacion,cliente,FInicio,Ffin)
...
set miCon=Server.CreateObject("ADODB.Connection")
 ConnectionString="DRIVER={SQL Server};SERVER=10.66.1.19;UID=sa;PWD=S0l1umF0rW;DATABASE=SIR"
	strSQL = "GSI_OPE_Finanzas_Takasago '"&Toperacion&"','"&cliente&"','"&FInicio&"','"&Ffin&"'"
...
snco="#FFFFFF"
  if err.number =0 then
      ...
      While Not  miRS.eof
	  'response.write(miRS("sNumPedimento"))
			HTML = HTML & "<tr>" & chr(13) & chr(10)
			
			HTML= HTML& celdadatos(miRS("sNumPedimento")) 
			HTML=HTML& celdadatos(miRS("Patente")) 
			HTML=HTML& celdadatos(miRS("cgas31")) 
			HTML=HTML& celdadatos(miRS("Agente")) 
			HTML=HTML& celdadatos(miRS("sCvePed")) 
			HTML=HTML& celdadatos(miRS("sClaveAduana")) 
			HTML=HTML& celdadatos(miRS("dFechaPago")) 
			HTML=HTML& celdadatos(miRS("IDFiscal")) 'ID
			HTML=HTML& celdadatos(miRS("Proveedor")) 'Proveedor
			HTML=HTML& celdadatos(miRS("ValorDolares")) ' Importe DLLS
			HTML=HTML& celdadatos(miRS("Facturas")) 'Facturas
			HTML=HTML& celdadatos(miRS("nFletesPed")) 
			HTML=HTML& celdadatos(miRS("nOtrosIncPed")) 
			HTML=HTML& celdadatos(miRS("nTipoCambio")) 
			HTML=HTML& celdadatos(miRS("nValorAduana")) 
			HTML=HTML& celdanumero(miRS("IGI/IGE")) 
			HTML=HTML& celdanumero(miRS("DTA")) 
			HTML=HTML& celdadatos(miRS("REC/IEPS U OTROS")) 
			HTML=HTML& celdadatos(miRS("Base para IVA")) 
			HTML=HTML& celdanumero(miRS("IVA")) 'Importe del IVA de mercancia en aduana
			HTML=HTML& celdanumero(miRS("PRV")) 
			HTML=HTML& celdadatos(miRS("FleteAM")) 
			HTML=HTML& celdadatos("") 'Proveedor
			HTML=HTML& celdadatos("") 'RFC Proveedor Flete
			HTML=HTML& celdanumero(miRS("FleteNacional16")) 
			HTML=HTML& celdanumero(miRS("FleteNacional11")) 
			HTML=HTML& celdadatos("") 'Autopista
			HTML=HTML& celdanumero("") 'IVA 16
			HTML=HTML& celdanumero("") 'IVA 11
			HTML=HTML& celdanumero("") 'IVA RETENIDO
			HTML=HTML& celdanumero(miRS("Maniobras"))
			HTML=HTML& celdanumero("")'Monto Anticipo
			HTML=HTML& celdanumero(miRS("Otros16"))
			HTML=HTML& celdanumero(miRS("Otros11"))
			HTML=HTML& celdanumero(miRS("Otros0"))
			HTML=HTML& celdanumero(miRS("IVA16"))			
			HTML=HTML& celdanumero(miRS("IVA11"))
			HTML=HTML& celdanumero("") 'Honorarios
			HTML=HTML& celdanumero("")' Servicios Com.
			HTML=HTML& celdanumero("") 'Iva 16
			HTML=HTML& celdanumero("") ' IVA11
			HTML = HTML &celdanumero("=SUMA(P"&contador&",Q"&contador&",T"&contador&",U"&contador&",V"&contador&",Y"&contador&",Z"&contador&",AA"&contador&",AB"&contador&",AC"&contador&",AD"&contador&",AE"&contador&",AF"&contador&",AG"&contador&",AH"&contador&",AI"&contador&",AJ"&contador&",AK"&contador&",AL"&contador&",AM"&contador&",AN"&contador&",AO"&contador&")") 'TOTAL
			HTML=HTML& celdadatos(miRS("TotalImpuestos")) 'Total Anticipo 
			HTML = HTML &celdadatos("=AP"&contador&"-AQ"&contador) 'Diferencia a cargo
			HTML = HTML & "</tr>" & chr(13) & chr(10)
			contador=contador+1
            miRS.movenext
        Wend
...
function retornaMYSQL(operacion,oficina,fechainicio,fechafin)
datos=""
if oficina="Todas" then 
	for ii=1 to 6
			...
			select case ii
				...
				if ii=1 then
					consulta=QuerySQL(oficina2,operacion)
				else
					consulta=consulta& " UNION ALL "&QuerySQL(oficina2,operacion)
			end if
		
	next  
else
	consulta=QuerySQL(oficina,operacion)
end if
...
dim Maniobra ,Desconsolidacion, Otros ,igi,dta,impma,prv,fleten,ivaflete,ivaretenido,ivaconceptos,honorarios,servcomp,ivaCG
		Maniobra=0
		Desconsolidacion=0 
		Otros=0
		igi=0
...
elseif op="S" then 
	...
	function QuerySQL(oficina,op)
		...
		sqlAct=sqlAct&" From "&oficina&"_extranet.ssdag"&op&"01 as i  "
		...
		sqlAct=sqlAct&" where  i.firmae01<>'' and i.firmae01 is not null and i.rfccli01='"&Vrfc&"' and i.fecpag01>='"&FechaI&"' and i.fecpag01<='"&FechaF&"'"
