	...
	Else
		...
		if mov = "i" Then
			header = header &	celdahead("Causa de Desvio Anticipos (Arbol de Perdidas)") &_
								celdahead("Fecha de la Solicitud de Anticipo") &_
...
function GeneraSQL
	...
	SQL = 	"SELECT  i.refcia01 AS 'referencia', " &_
			...
			Else
				SQL = SQL & "IF(c.frec01 IS NULL OR c.frec01 = '0000-00-00' , 'No Capturada', DATE_FORMAT(c.frec01, '%d-%m-%Y')) AS 'fentrada', " &_
	...
	SQL = SQL & kpi("", "c.frev01", "c.fdsp01") & "AS 'KPIGRK', " &_
			...
			"WHERE i.firmae01 IS NOT NULL AND i.firmae01 <> '' AND i.cveped01 <> 'R1' " &_
			"AND ((c.fdsp01 >= '" & DateI & "' AND c.fdsp01 <= '" & DateF & "') OR c.fdsp01 = '00-00-0000') " & condicion &_
