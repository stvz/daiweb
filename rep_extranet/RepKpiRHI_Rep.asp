	...
	Else
		...
		header = 	"<table  width = ""778""  border = ""2"" cellspacing = ""0"" cellpadding = ""0"">" &_
							"<tr bgcolor = ""#006699"" class = ""boton"">" &_
								...
								if mov="e" then
								header =header & celdahead("Total bultos") &_
										celdahead("Numeros de contenedor")
								end if 
				header = 		header &		celdahead("Pedimento") &_
								celdahead("Cliente") &_
								celdahead("Ejecutivo")
		if mov = "i" Then
		...
		if mov = "i" Then
			header = header &	celdahead("Total de Contenedores / Bultos") 
			'else
			
			'header = header &	celdahead("Destino de la Mercanc&iacute;a")
		End If
		...
		if mov = "i" Then
			header = header &	celdahead("Anticipo") &_
								celdahead("Fecha en que fue Depositado el Anticipo") 
		End If
		header = header &	"</tr>"
		datos = ""
		Referencia = ""
		...
		Do Until RSops.EOF
			...
			datos = datos &	"<tr>" &_
								celdadatos(Referencia) 
								if mov="e" then
								datos = datos & celdadatos(RSops.Fields.Item("bultos").value) &_
										celdadatos(RSops.Fields.Item("Contenedores").value)
								end if 
						datos= datos &celdadatos(RSops.Fields.Item("pedimento").Value) &_
		...
		'	End If
			...
			datos = datos &		celdadatos(RSops.Fields.Item("FCG").Value) &_
								...
								celdadatos(obs)
			if mov = "i" Then
				datos = datos & celdadatos(importe) &_
								celdadatos(RSops.Fields.Item("fdeposito").Value)
...
function GeneraSQL
	SQL = ""
	condicion = filtro
	SQL = 	"SELECT group_concat(distinct conte.numcon40 ) as Contenedores, i.refcia01 AS 'referencia', " &_
	...
	SQL = SQL & kpi("", "c.frev01", "c.fdsp01") & "AS 'KPIGRK', " &_
			...
			"left join " & strOficina & "_extranet.sscont40  as conte on conte.refcia40  =i.refcia01 and conte.adusec40  =i.adusec01 and conte.patent40  =i.patent01 "&_
			"WHERE i.firmae01 IS NOT NULL AND i.firmae01 <> '' " &_
