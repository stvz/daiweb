...
if  Session("GAduana") = "" then
	...
	Else
									...
									"<font color=""#000066"" size=""4"" face=""Arial, Helvetica, sans-serif"">" &_
										"<td colspan=""" & nocolumns & """>" &_
											"<p align=""left""><font color=""#000066"" size=""3"" face=""Arial, Helvetica, sans-serif"">" &_
												"BOOKLET EXTRA COSTOS DE EXPORTACIÓN.." &_
											"</font></p>" &_
											"<p>" &_
											"</p>" &_
											"<p>" &_
		...
		header = 			"<tr class = ""boton"">" &_
								...
								celdahead("Nombre de Proveedor") &_
								celdahead("Fecha de Embarque") &_
								celdahead("Concepto") &_
								celdahead("Dias de demora o estadia") &_
								celdahead("Monto") &_
								celdahead("Moneda")
				header = header &	"</tr>"
		...
		Do Until RSops.EOF
 				...
 				datos = datos&"<tr> "&_
								...
								celdadatos(RSops.Fields.Item("Planta").Value) &_
								celdadatos(RSops.Fields.Item("Contenedores").Value) &_
								celdadatos(RSops.Fields.Item("Nombre de Proveedor").Value) &_
								celdadatos(RSops.Fields.Item("Fecha de pago").Value) &_
								celdadatos(RSops.Fields.Item("Concepto").Value) &_
								celdadatos(RSops.Fields.Item("Dias de Demora o Estadia").Value) &_
								celdadatos(RSops.Fields.Item("Monto").Value) &_
								celdadatos(RSops.Fields.Item("Moneda").Value)
				datos = datos &	"</tr>"
...
function GeneraSQL
	...
	For i = 0 to 3
		...
		End Select
			...
			SQL = SQL & "DATE_FORMAT(RE.femb01,'%d/%m/%Y') as 'Fecha de Embarque', " & chr(13) & chr(10)
			...
			SQL = SQL & "and CP.clav21 not in(31,390,389,406,135,116,25,319,397,1,2,388,407,408) "& chr(13) & chr(10)  
			elseif aduanaTmp="sap" then
			SQL = SQL & "and CP.clav21 not in(188,168,175,17,2,258,70,13,178,12,151,80,127,1) "& chr(13) & chr(10)  
			elseif aduanaTmp="dai" then
			SQL = SQL &"and CP.clav21 not in(1,11,2,31,127,239)  "& chr(13) & chr(10) 
			elseif aduanaTmp="lzr" then
			SQL = SQL &"and CP.clav21 not in(1,297,397)  "& chr(13) & chr(10) 
			end if 
...
<HTML>
	<HEAD>
		<TITLE>::.... REPORTE DE EXTRACOSTOS DE EXPORTACION UNILEVER .... ::</TITLE>
	</HEAD>
	<BODY>
	<%=html%>
	</BODY>
