...
if  Session("GAduana") = "" then
	...
	Else
		if Tiporepo = 2 Then
			Response.Addheader "Content-Disposition", "attachment; filename=""Reporte de Operaciones " & noficina & " " & mid(movi,1,4) & """.xls"
			Response.ContentType = "application/vnd.ms-excel"
			
		End If
		info = 	"<table  width = ""100%""  border = ""0"" cellspacing = ""0"" cellpadding = ""0"">" &_
							"<tr>" &_								
								"<td style='vertical-align:middle;text-align:center;' colspan=""" & nocolumns & """>" &_
									"<center>" &_
		...
		header = 	"<table  width = ""778""  border = ""2"" cellspacing = ""0"" cellpadding = ""0"">" &_
							"<tr bgcolor = ""#333399"" class = ""boton"">" &_
								celdahead("No") &_
		...
		if mov = "i" Then
			header = header &	celdahead("Recepci&oacute;n de Cuenta de gastos")
		end if			
		
		header = header &		celdahead("Tiempos  de Facturaci&oacute;n Despacho vs Recepci&oacute;n  Cgastos") &_
		...
		contador=1
		division=0
		estilo="style='background-color:#A6CAF0;'"
		Do Until RSops.EOF
...
function celdahead(texto)
	cell = "<td bgcolor = ""#333399"" width=""100"" nowrap>" &_
				"<center>" &_
					"<strong>" &_
...
function GeneraSQL
	...
	SQL = 	"SELECT  i.refcia01 AS 'referencia', " &_
			...
			"IF(c.fpre01 IS NULL OR c.fpre01 = '0000-00-00', if((select ip.cveide11  from rku_extranet.ssiped11 as ip where ip.refcia11 =i.refcia01 and ip.cveide11 ='RO')='RO',DATE_FORMAT(i.fecpag01,'%d-%m-%Y'), 'No Capturada'), DATE_FORMAT(c.fpre01,'%d-%m-%Y')) AS 'fprev',  " &_
	...
	SQL = SQL & kpi("", "c.frev01", "c.fdsp01") & "AS 'KPIGRK', " &_
			...
			"LEFT JOIN trackingbahia.bit_soia as bs ON bs.frmsaai01 = i.refcia01 " & _
			"WHERE i.firmae01 IS NOT NULL AND i.firmae01 <> '' AND i.cveped01 <> 'R1' " &_
