<html>
	<head>
		...
		dim oficina,cvesoficina,validacion,codigo, cvecli, adu, SqlAct
		cvecli= ""
		codigo = ""
		oficina=""
		cvesoficina=""
...
function genera_registros(det,tipope,finicio,ffinal)
	...
	dim acumdecseg, acumdecotr, acumvadu, acumpre, acumigi, acumdta, acumeci, acumimp, acumcuo, acumiva, acumtadu, tabla
	...
	Select Case strTipoOperaciones
		Case 1
			tabla = "ssdagi01"
		Case 2
			tabla = "ssdage01"
	...
	genera_html "e","Fraccion","center","'String'"
	...
	Select Case strTipoOperaciones
		...
		Case 2
		genera_html "e","Total Factura","center","'String'"
		Case 3
		genera_html "e","Valor Moneda Extranjera/Total Factura","center","'String'"
	End Select
	...
	Select Case strTipoOperaciones
		...
		Case 2
			genera_html "e","Pais Comprador","center","'String'"
			genera_html "e","Pais Destino","center","'String'"
		Case 3
			genera_html "e","Pais Vendedor/Comprador","center","'String'"
			genera_html "e","Pais Origen/Destino","center","'String'"
	End Select
	...
	escribe_query tabla
		...
		If strTipoOperaciones = 3 Then
			tabla = "ssdage01"
			sqlAct = sqlAct & " UNION ALL "
			escribe_query tabla
			sqlAct = sqlAct & " order by patped"
		Else
			sqlAct = sqlAct & " order by CONCAT_WS('-',i.patent01,i.numped01)"	
		End If
		...
		while not act2.eof
			...
			genera_html "d" & color,act2.fields("TipoOp").value,"right","'Number'"
			...
			genera_html "d" & color,act2.fields("fraccion").value,"right","'String'"
		...
		Select Case strTipoOperaciones
			...
			Case 2
				genera_html "e","Total Exportaciones","right","'String'"
			Case 3
				genera_html "e","Total Operaciones","right","'String'"
		End Select
...
Sub escribe_query(tabla)

sqlAct= sqlAct & " SELECT " &_
		...
		"fr.fraarn02 as fraccion, "
		If tabla = "ssdagi01" then
			sqlAct = sqlAct & "1 as 'TipoOp', " 
		Else
			sqlAct = sqlAct & "0 as 'TipoOp', " 
		End If
		sqlAct = sqlAct & "i.cveped01 as 'CvePed', " &_
		...
		"IF(i.tipcam01 IS NOT NULL and i.tipcam01 <> 0,ROUND(i.tipcam01,5),0) as 'TipoCam', " &_
		"fac.terfac39 as 'incoterm', " &_
		"IF(fac.valmex39 IS NOT NULL and fac.valmex39 <> '',ROUND(fac.valmex39,2),0)as 'valormext', " &_
		"IF(c.dmer01 IS NOT NULL and c.dmer01 <> '',REPLACE(REPLACE(c.dmer01,'\r',''),'\n',''),'----') as 'descrip', " &_
		"ROUND(IF(fac.valmex39 IS NULL,0,fac.valmex39)*IF(i.tipcam01 IS NULL,1,i.tipcam01)*IF(i.factmo01 IS NULL,i.factmo01,1),0) as 'valormn', " &_
		"IF(i.fletes01 IS NULL OR i.fletes01 = '',0,ROUND(i.fletes01,2)) as 'IncFletes', " &_
		"IF(i.segros01 IS NULL OR i.segros01 = '',0,ROUND(i.segros01,2)) as 'IncSeguros', " &_
		"IF(i.incble01 IS NULL OR i.incble01 = '',0,ROUND(i.incble01,2)) as 'IncOtros', " &_
		"0 as 'DecFletes', " &_
		"0 as 'DecSeguros', " &_
		"0 as 'DecOtros', " 
		if tabla = "ssdagi01" Then
			sqlAct = sqlAct & "ROUND(((IF(i.tipcam01 IS NULL,1,i.tipcam01) * IF(i.valmer01 IS NULL,0,i.valmer01) * IF(i.factmo01 IS NULL,0,i.factmo01)) + IF(i.fletes01 IS NULL,0,i.fletes01)) +" &_
			"IF(i.segros01 IS NULL,0,i.segros01) + IF(0 = 0, 0, 0) - IF(0 = 0, 0, 0) - 0 -" &_
			"IF(0 = 0, 0, 0),0) as 'valoradu', "
		else
			sqlAct = sqlAct & "0 as 'valoradu', "
		End If
		sqlAct = sqlAct & "IF(con1.import36 IS NOT NULL AND con1.import36 <> '',ROUND(con1.import36,2),0) as 'Pre', " &_
		"IF(con3.import36 IS NOT NULL AND con3.import36 <> '',ROUND(con3.import36,2),0) as 'IGI', " &_
		"IF(con4.import36 IS NOT NULL AND con4.import36 <> '',ROUND(con4.import36,2),0) as 'DTA', " &_
		"IF(con2.import36 IS NOT NULL AND con2.import36 <> '',ROUND(con2.import36,2),0) as 'ECI', " &_
		"IF(con7.import36 IS NOT NULL AND con7.import36 <> '',ROUND(con7.import36,2),0) as 'OtrosImp', " &_
		"IF(con6.import36 IS NOT NULL AND con6.import36 <> '',ROUND(con6.import36,2),0) as 'CuotCompen', " &_
		"IF(con5.import36 IS NOT NULL AND con5.import36 <> '',ROUND(con5.import36,2),0) as 'IVA', " &_
		...
		"TRIM(REPLACE(REPLACE(CONCAT(TRIM(CAST(prov.cvepro22 AS CHAR)),' ',TRIM(prov.nompro22)),'CANCELADO',''),'*','')) as 'Proveedor', " &_
		"i.cvepvc01 as 'Pais_Vendedor', " &_
		"fr.paiori02 as 'Pais_Origen' " &_
		"FROM rku_extranet." & tabla & " as i " &_
		...
		"group by i.refcia01, fac.numfac39, c.dmer01 "
