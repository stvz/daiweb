	...
	'******************************************************************************************************************************
		...
		if strTipoPedimento  = "1" then
			...
			strSQL =   " SELECT concat(concat(concat(concat(concat(concat(ltrim(substring(year(FECPAG01),3,2)),'-'),adusec01 ),'-'),PATENT01),'-'),NUMPED01) as IMPORTA, " & _
                   ...
                   "        TIPCAM01 as TipoCambio, " & _
                   "        FACTMO01, " & _
				   "        round((d05artic.vafa05/d05artic.caco05),2) as xPreUnitario " & _
                   "  from ssdagi01  " & _
				   "        left join ssfact39 on ssfact39.refcia39 = ssdagi01.refcia01   " & _
				   "		left join d05artic on d05artic.refe05 = ssdagi01.refcia01 and ssfact39.numfac39 = d05artic.fact05  " & _
				   "		left join ssfrac02 on ssdagi01.refcia01 = ssfrac02.refcia02  and d05artic.frac05 = ssfrac02.fraarn02  and ssfrac02.ordfra02 = d05artic.agru05  " & _
                   "  where          " & _
                   "        firmae01 <> ''   and  " & _
                   "       fecpag01 >= '"&FormatoFechaInv(strDateIni)&"' AND " & _
                   "       fecpag01 <= '"&FormatoFechaInv(strDateFin)&"' AND " & _
		...
		if strTipoPedimento  = "2" then
			...
			strSQL =   " SELECT concat(concat(concat(concat(concat(concat(ltrim(substring(year(FECPAG01),3,2)),'-'),adusec01 ),'-'),PATENT01),'-'),NUMPED01) as IMPORTA, " & _
					...
					"        round((d05artic.vafa05/d05artic.caco05),2) as xPreUnitario " & _
                    "  from ssdage01  " & _
					"        left join ssfact39 on ssfact39.refcia39 = ssdage01.refcia01   " & _
				    "		left join d05artic on d05artic.refe05 = ssdage01.refcia01 and ssfact39.numfac39 = d05artic.fact05  " & _
				    "		left join ssfrac02 on ssdage01.refcia01 = ssfrac02.refcia02  and d05artic.frac05 = ssfrac02.fraarn02  and ssfrac02.ordfra02 = d05artic.agru05  " & _
                    "  where          " & _
                    "        firmae01 <> ''   and  " & _
                    "       fecpag01 >= '"&FormatoFechaInv(strDateIni)&"' AND " & _
                    "       fecpag01 <= '"&FormatoFechaInv(strDateFin)&"' AND " & _
		...
		if not trim(strSQL)="" then
           ...
           While NOT RsRep.EOF 'recorremos el cursor de operaciones
				  ...
				  xPreUni = RsRep.Fields.Item("xPreUnitario").Value
                         ...
                         if not RsDetMercancias.eof then ' si tiene fracciones
                            While not RsDetMercancias.eof ' recorremos las fracciones
                                ...
                                'dblDTA = RsDetMercancias.Fields.Item("DTA").Value
                                 ...
                                 'if RsDetMercancias.Fields.Item("caco05").Value > 0 then
                                 '   xPreUni  = RsDetMercancias.Fields.Item("vafa05").Value/RsDetMercancias.Fields.Item("caco05").Value
                                 'else
                                 '   xPreUni  = RsDetMercancias.Fields.Item("vafa05").Value
                                 'end if

                                 'xPreUni  = RsDetMercancias.Fields.Item("vafa05").Value
                                 ' if xCant > 0 then
                                    ' xPreUni  = RsDetMercancias.Fields.Item("ValorComercial").Value / xCant
									
                                 ' else
                                    ' xPreUni  = RsDetMercancias.Fields.Item("ValorComercial").Value
                                 ' end if

								 xFactmo    =  RsDetMercancias.Fields.Item("FACMONEDA").Value
								
                                 'xPreUni  = RsDetMercancias.Fields.Item("vafa05").Value * xFactmo
                                 ...
                                 ' xAnio    = RsDetMercancias.Fields.Item("anio").Value
                                 ' if xAnio >= 1900 then
                                 ...
                                 '     end if
                                 ' else
                                 '     xAnio    = RsDetMercancias.Fields.Item("aniomerc").Value
                                 '     if xAnio >= 1900 then
                                 '         xFecpag  = RsDetMercancias.Fields.Item("ffactp05").Value
                                 '     else
                                 '         xFecpag  = "0000-00-00"
                               ...
                               strHTML = strHTML&"<td nowrap><font color=""#000000"" size=""1"" face=""Arial, Helvetica, sans-serif""> " & xStrUniMed       & "    </font></td>" & chr(13) & chr(10) 'UNIDAD       Unidad del numero de parte
	  	                         strHTML = strHTML&"<td nowrap><font color=""#000000"" size=""1"" face=""Arial, Helvetica, sans-serif""> " & RsRep.Fields.Item("xPreUnitario").Value & "    </font></td>" & chr(13) & chr(10) 'PRECIO       Precio Unitario
