...
<%
    ...
    function DespliegaEncabezado()
       ...
       strHTML = strHTML & "<strong><font color=""#969696"" size=""3"" face=""Arial, Helvetica, sans-serif""> REPORTE DE TRACKING MARITIMO" & " </font></strong>"
...
<%
    ...
    'BotonOtrosFiltros - Otros Flitros de captura libre
                ...
                if ltrim(strTipoFecha) = "DAITTS" OR ltrim(strTipoFecha) = "DFPAG" then  ' Selecciona la fecha de ITTS
                    ...
                    IF ltrim(strTipoFecha) = "DAITTS" then
                        strSQL = " SELECT SSDAGI01.REFCIA01  AS REFERENCIA,      " & _
                                 ...
                                 " WHERE modo01 = 'T' and  frec01 >= '"&FormatoFechaInv(strDateIni)&"' AND        " & _
                     ...
                     ELSE
                          strSQL = " SELECT SSDAGI01.REFCIA01  AS REFERENCIA,      " & _
                                   ...
                                   "        ADUDES01                               " & _
                                   " FROM C01REFER INNER JOIN SSDAGI01 ON REFCIA01= C01REFER.REFE01      " & _
                                   " WHERE modo01 = 'T' and  fecpag01 >= '"&FormatoFechaInv(strDateIni)&"' AND        " & _
                  ...
                  strSQL = " SELECT SSDAGI01.REFCIA01  AS REFERENCIA,      " & _
                           ...
                           "        ADUDES01                               " & _
                           " FROM C01REFER INNER JOIN SSDAGI01 ON REFCIA01= C01REFER.REFE01      " & _
                           " WHERE modo01 = 'T' and  firmae01   <> ''        AND                            " & _
                  ...
                  if strTipoOtrosFiltros = "Modelo" then ' filtro por modelo
                    'strfiltrosrestantes
                     strSQL = " SELECT SSDAGI01.REFCIA01  AS REFERENCIA,      " & _
                              ...
                              " WHERE modo01 = 'T' and  cpro05 = '"&LTRIM(strfiltrosrestantes)   & "' " & Permi & _
                              " ORDER BY ETA_PORT2, ETA_PORT  "
                  else
                     if strTipoOtrosFiltros = "Descripcion" then ' filtro po Descripcion de mercancia
                         strSQL = " SELECT SSDAGI01.REFCIA01  AS REFERENCIA,      " & _
                              ...
                              " FROM C01REFER INNER JOIN SSDAGI01 ON REFCIA01= C01REFER.REFE01  " & _
                              "               INNER JOIN D05ARTIC ON REFE01=REFE05 " & _
                              " WHERE modo01 = 'T' and desc05 LIKE'%" & LTRIM(strfiltrosrestantes)& "%' " & Permi & _
                              " ORDER BY ETA_PORT2, ETA_PORT  "
                     end if
                  end if
                    'strfiltrosrestantes   = trim(request.Form("txtfiltrosrestantes"))
                   ...
                   '-------------------------------------------------
                                  ...
                                  strSQL = " SELECT SSDAGI01.REFCIA01  AS REFERENCIA,      " & _
                                           ...
                                           " WHERE modo01 = 'T' and C01REFER.REFE01 <> '' " & Permi & strCadFiltroLinNav & strCadFiltroModal & strCadFiltroProv & _
        ...
        'strSQL= " SELECT SSDAGI01.REFCIA01  AS REFERENCIA,              C01REFER.PTOEMB01  AS PORT_LOADING,            C01REFER.PAISEM01  AS VESSEL_LOADING,          SSDAGI01.adusec01  AS PORT_DISCHARGE,          C01REFER.Naim01    AS SHIPPING_LINE,           SSDAGI01.REGBAR01  AS VESSEL,                  SSDAGI01.PATENT01,                             CONCAT(SSDAGI01.PATENT01, CONCAT( '-',SSDAGI01.NUMPED01 ) ) AS IMPORT_DOCUMENT,         SSDAGI01.CVEPRO01  AS PROVEEDOR,               C01REFER.feta01    AS ETA_PORT,                SSDAGI01.fecent01  AS ETA_PORT2,               SSDAGI01.fecent01,                             C01REFER.frev01    AS REVALIDACION,            C01REFER.fcot01    AS RESQUEST_DUTIES,         C01REFER.fpre01    AS PREVIO,                  C01REFER.fdsp01    AS DATE_CUSTOM,             SSDAGI01.cvemts01  AS MODALIDAD,               SSDAGI01.desf0101  AS FACTURAS,                firmae01,                                      frec01 as FecITTS,                             feorig01 as FECETDLOAD,                        sum(if( n_etapa=13,1,0)  ) as llegada,          cbuq01,                                        CVEPED01,                                      cveptoemb,                                     ADUDES01                                FROM C01REFER INNER JOIN SSDAGI01 ON REFCIA01= C01REFER.REFE01            LEFT JOIN SAP_STATUS.ETXCOI                                      ON REFCIA01=SAP_STATUS.ETXCOI.C_REFERENCIA AND SAP_STATUS.ETXCOI.N_ETAPA = 13 WHERE modo01 = 'T' and C01REFER.REFE01 <> ''  AND cvecli01 =1283     AND   ( isnull( f_fecha )  or f_fecha >= '2009/5/20' ) AND REFCIA01 = 'SAP09-0152'   GROUP BY REFCIA01  ORDER BY ETA_PORT2, ETA_PORT  "
        ...
        'response.end
         ...
         '*********************************************************************************************************************************************************************************************************************************************
           ...
           strSQLPlSTD = " SELECT D.n_orden as orden,     " & _
                         ...
                         "      ETAPS AS E ,  " & _
                         "      ETAPS AS B    " & _
                         " INNER JOIN D01STD ON E.N_ETAPA= ETPINI01 and tipoadu='MARITIMA' " & _
           ...
           StdDSPCP      = 0 'std para DESPACHO A CONTRIMODAL
           StdRAILCP     = 0 'std para RAIL A CONTRIMODAL
           StdCPWH       = 0 'std para CONTRIMODAL A WAREHOUSE
           ...
           tipoStdDSPCP      = 1 'tipo de dias de std DESPACHO A CONTRIMODAL
           tipoStdRAILCP     = 1 'tipo de dias de std RAIL A CONTRIMODAL
           tipoStdCPWH       = 1 'tipo de dias de std CONTRIMODAL A WAREHOUSE
                    ...
                    tipoStdATAPORTDSP = RsPlSTD.Fields.Item("tipod").Value
                     ...
                     if RsPlSTD.Fields.Item("inicio").Value = "DSP" and RsPlSTD.Fields.Item("fin").Value = "LLP" then
                        ...
                        tipoStdDSPWH = RsPlSTD.Fields.Item("tipod").Value
                         ...
                         if RsPlSTD.Fields.Item("inicio").Value = "DSP" and RsPlSTD.Fields.Item("fin").Value = "CP" then
                            StdDSPCP     = RsPlSTD.Fields.Item("dias").Value
                            tipoStdDSPCP = RsPlSTD.Fields.Item("tipod").Value
                         else
                             if RsPlSTD.Fields.Item("inicio").Value = "ATDRAIL" and RsPlSTD.Fields.Item("fin").Value = "CP" then
                                StdRAILCP     = RsPlSTD.Fields.Item("dias").Value
                                tipoStdRAILCP = RsPlSTD.Fields.Item("tipod").Value
                             else
                                  if RsPlSTD.Fields.Item("inicio").Value = "CP" and RsPlSTD.Fields.Item("fin").Value = "LLP" then
                                    StdCPWH     = RsPlSTD.Fields.Item("dias").Value
                                    tipoStdCPWH = RsPlSTD.Fields.Item("tipod").Value
                                  end if
                             end if
                         end if
                     end if
...
''                if (RsRep.Fields.Item("llegada").Value = 0) then ' inicio filtro de llegada
                ' ya tenemos los registros a nivel pedimento, ahora vamos por todos los campos a nivel pedimento restantes
                     ...
                     '    'if StrCvePed <> "" and ltrim(StrCvePed) = "R1" then
		...
		'         end if
                         ...
                         ' Vamos por las mercancias
                           ...
                           intcontDescMerc = 1
                             While NOT RMercancias.EOF
                                 ...
                                 '   intcontped = intcontped + 1
                                 'end if
                                 'if RMercancias.Fields.Item("cpro05").Value <> "" then
                                 '    if intcontemp = 1 then
                                 '       strModelo     = RMercancias.Fields.Item("cpro05").Value
                                 '    else
                                 '       strModelo     = strModelo & ", "& RMercancias.Fields.Item("cpro05").Value
                                 ...
                                 '    intcontemp = intcontemp + 1
                                 'end if
                                 'if RMercancias.Fields.Item("tpmerc05").Value <> "" then
                                 '    if intcontDescFra = 1 then
                                 '       strDescCode   = RMercancias.Fields.Item("tpmerc05").Value
                                 '    else
                                 '       strDescCode   = strDescCode & ", "& RMercancias.Fields.Item("tpmerc05").Value
                                 '    end if
                                 '    intcontDescFra = intcontDescFra + 1
                                 'end if
                                 'if RMercancias.Fields.Item("desc05").Value <> "" then
                                 '    if intcontDescMerc = 1 then
                                 '       strDescMerc   = RMercancias.Fields.Item("desc05").Value
                                 '    else
                                 '       strDescMerc   = strDescMerc & ", "& RMercancias.Fields.Item("desc05").Value
                                 '    end if
                                 '    intcontDescMerc = intcontDescMerc + 1
                                 'end if
                         ...
                         'else
                             ...
                             if StrRefer <> "" then
                                 ...
                                 strSQL = " SELECT NUMFAC39,  CAST(FECFAC39 AS CHAR)  FECFAC39  " & _
                                          " FROM SSFACT39 " & _
                                          " WHERE REFCIA39='" & ltrim(StrRefer) & "'"
                         ...
                         'end if
                                          ...
                                          'if intColumna = 860 then
                                          '  'Response.Write(RsRep.Fields.Item("REFERENCIA").Value)
                                          '  'Response.Write(StrfECINVOICETemp)
                                          '  Response.End
                                          'end if
                         ...
                         if StrRefer <> "" then
                             ...
                             if not RContenedores.eof then
                               While NOT RContenedores.EOF
                                       ...
                                       if isdate( RsRep.Fields.Item("FECETDLOAD").Value ) then
                                          StrPETD_LOAD 	 = YEAR( RsRep.Fields.Item("FECETDLOAD").Value ) & Pd(Month( RsRep.Fields.Item("FECETDLOAD").Value ),2) & Pd(DAY( RsRep.Fields.Item("FECETDLOAD").Value ),2)  'FECHA DE SALIDA DE ORIGEN
                                       else
                                          StrPETD_LOAD = RsRep.Fields.Item("FECETDLOAD").Value  'FECHA DE SALIDA DE ORIGEN
                                       end if
                                       ...
                                       'Response.Write("RsRep.Fields.Item(FECETDLOAD).Value")
                                       'Response.Write(RsRep.Fields.Item("FECETDLOAD").Value)
                                       'Response.Write("StdEtdLoad")
                                       'Response.Write(StdEtdLoad)
                                       ...
                                       'StrETA_CUSTOM_CLEARANCE = SumarDiasSinFinSemana( StrPETA_PORT_AUX , 2)
                                       if isdate(DFechEntAux) then
                                         'StrETA_CUSTOM_CLEARANCE = SumarDiasSinFinSemana( DFechEntAux , 2)
                                         'StrETA_CUSTOM_CLEARANCE = SumarDiasSinFinSemana( SumarDiasSinFinSemana( DFechEntAux , StdATAPORTDSP) , diaRmkATAPORT)
                                         StrETA_CUSTOM_CLEARANCE = SumarDias( SumarDias( DFechEntAux , StdATAPORTDSP,tipoStdATAPORTDSP) , diaRmkATAPORT,tipoRmkATAPORT)
                                       else
                                         'StrETA_CUSTOM_CLEARANCE = SumarDiasSinFinSemana( StrPETA_PORT_AUX , 2)
                                         'StrETA_CUSTOM_CLEARANCE = SumarDiasSinFinSemana( SumarDiasSinFinSemana( StrPETA_PORT_AUX , StdATAPORTDSP) , diaRmkATAPORT)
                                         StrETA_CUSTOM_CLEARANCE = SumarDias( SumarDias( StrPETA_PORT_AUX , StdATAPORTDSP,tipoStdATAPORTDSP) , diaRmkATAPORT,tipoRmkATAPORT)
                                       end if
                                       'Response.End


                                       if StrModalidad = "CARRETERO." then
                                             StrETA_C_P = ""
                                             if isdate( StrETA_CUSTOM_CLEARANCE ) and isdate( RsRep.Fields.Item("DATE_CUSTOM").Value ) then
                                                 IndFila = DateDiff("d", StrETA_CUSTOM_CLEARANCE , RsRep.Fields.Item("DATE_CUSTOM").Value)
                                                 if IndFila = 0 then
                                                    StrColorfila = 1
                                                    'StrETA_W_H_AUX = SumarDiasSinFinSemana( SumarDiasSinFinSemana(StrETA_CUSTOM_CLEARANCE , StdDSPWH) , diaRmkDSP)
                                                    StrETA_W_H_AUX = SumarDias( SumarDias(StrETA_CUSTOM_CLEARANCE , StdDSPWH,tipoStdDSPWH) , diaRmkDSP,tipoRmkDSP)
                                                 else
                                                    'StrETA_W_H_AUX = SumarDiasSinFinSemana( SumarDiasSinFinSemana(RsRep.Fields.Item("DATE_CUSTOM").Value , StdDSPWH) , diaRmkDSP)
                                            ...
                                            if strATDRAIL <> "" and isdate(strATDRAIL) then ' si existe fecha de ferrocarril
                                                 ...
                                                 if isdate( StrETA_CUSTOM_CLEARANCE ) and isdate( RsRep.Fields.Item("DATE_CUSTOM").Value ) then
                                                     ...
                                                     if IndFila = 0 then
                                                        ...
                                                        'StrETA_C_P = SumarDiasSinFinSemana(SumarDiasSinFinSemana(StrETA_CUSTOM_CLEARANCE , StdDSPCP) , diaRmkDSP)
                                                        StrETA_C_P = SumarDias(SumarDias(StrETA_CUSTOM_CLEARANCE , StdDSPCP,tipoStdDSPCP) , diaRmkDSP,tipoRmkDSP)
                                                     else
                                                        'StrETA_C_P = SumarDiasSinFinSemana( RsRep.Fields.Item("DATE_CUSTOM").Value , 3)
                                                        StrETA_C_P = SumarDias(SumarDias( RsRep.Fields.Item("DATE_CUSTOM").Value , StdDSPCP,tipoStdDSPCP) , diaRmkDSP,tipoRmkDSP)
                                                        if IndFila < 0 then
                                                            StrColorfila = 2
                                                         else
                                                            StrColorfila = 3
                                            ...
                                            if isdate(strATAC_P) then
                                               ...
                                               if IndFila = 0 then
                                                  ...
                                                  'StrETA_W_H_AUX = SumarDiasSinFinSemana( SumarDiasSinFinSemana(StrETA_C_P , StdCPWH) , StdCPWH)
                                                  StrETA_W_H_AUX = SumarDias( SumarDias(StrETA_C_P , StdCPWH,tipoStdCPWH) , diaRmkCP, tipoRmkCP)
                                               else
                                                  'StrETA_W_H_AUX = SumarDiasSinFinSemana(strATAC_P , 1)
                                                  'StrETA_W_H_AUX = SumarDiasSinFinSemana( SumarDiasSinFinSemana(strATAC_P , StdCPWH) , StdCPWH)
                                                  StrETA_W_H_AUX = SumarDias( SumarDias(strATAC_P , StdCPWH,tipoStdCPWH) , diaRmkCP, tipoRmkCP)
                                       ...
                                       'if isdate( strATASPLTMP ) then
                                       '   strATASPL = YEAR( strATASPLTMP ) & Pd(Month( strATASPLTMP ),2) & Pd(DAY( strATASPLTMP ),2)  'FECHA DE NOTIFICACION
                                       'else
                                       '   strATASPL = strATASPLTMP 'FECHA DE NOTIFICACION
                                       'end if
                                       ...
                                       'Response.Write("DFechEntAux")
                                       'Response.Write(DFechEntAux)
                                       'Response.Write("<br >StrETA_W_H_AUX")
                                       'Response.Write(StrETA_W_H_AUX)
                                       'Response.End
                             ...
                             'Response.End
                                       ...
                                       'StrPPORT_OF_DISCHARGE = RsRep.Fields.Item("ADUDES01").Value  'aduana de destino (en la que llega la mercancia directo de Origen)
                                       'strADUDESPACHO = RsRep.Fields.Item("PORT_DISCHARGE").Value  'aduana aduana de despacho
...
''                end if ' fin filtro de llegada
                    ...
                    'Response.Write(enproceso( Session("GAduana") ))
                    'Response.Write("banCargaRun="&banCargaRun)
