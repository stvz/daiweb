...
permi = PermisoClientes(Session("GAduana"),strPermisos,"CG.clie31")


if not permi = "" then
  permi = "  and (" & permi & ") "
end if
'response.write(permi&"000--->>")
...
'response.write(permi&" ---> Permisos del Ususario")
   ...
   strSQL ="select " &_
	...
	" HAVING sum(If(EP.deha21='A', DP.mont21,(DP.mont21)*-1)) > 0 " &_
  ...
  End If
   ...
   While NOT Rsio.EOF
    ' Modificacion agregada por Manuel Alejandro Estévez Fernández Marzo 4, 2013
    ' Se agrega consulta al SIR para verificar que la 
