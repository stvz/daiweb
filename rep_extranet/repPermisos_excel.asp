...
'                   oSS.Worksheets(intcontEjec + 1).Name = "ANTONIETA CAMPUZANO"
             ...
             strSQLPermi =  " SELECT         PERMI01         AS PERMISO,                 " & _
                            ...
                            " WHERE "&permi& "  AND PERMI01 <> 'I802342006'  AND PERMI01 <> 'I500752006' AND PERMI01 <> 'I500022007'  "  & _
                            " ORDER BY  PERMISO , FECHASALIDA , PRODUCTO, CANTIDAD, OBSERV01, EJEC01, cantidadfac "
                            '" ORDER BY  PRODUCTO, PERMISO, FECHASALIDA, CANTIDAD, OBSERV01, EJEC01, cantidadfac "
             ...
             '               "                   LEFT JOIN SSIPAR12           ON NUMPER12   = PERMI01   AND PERMI01 is not null  " & _
             '               "                   LEFT JOIN SSFRAC02           ON REFCIA02   = REFCIA12  AND ORDFRA02  = ORDFRA12 " & _
             '               "                   LEFT JOIN SSDAGI01 ON REFCIA01=REFCIA02  " & _
             '               "    WHERE "&permi & _
             '               "    UNION                                                   " & _
             '               "    SELECT  PERMI01         AS PERMISO,                     " & _
             '               "                      FECSAL01        AS FECHASALIDA,       " & _
             '               "                      PRODUCT01       AS PRODUCTO,          " & _
             '               "                      CANT01          AS CANTIDAD,          " & _
             '               "                      OBSERV01,                             " & _
             ...
             '               "                   LEFT JOIN SSIPAR12           ON NUMPER12   = PERMI01   AND PERMI01 is not null  " & _
             '               "                   LEFT JOIN SSFRAC02           ON REFCIA02   = REFCIA12  AND ORDFRA02  = ORDFRA12 " & _
             '               "                   LEFT JOIN SSDAGE01           ON REFCIA01=REFCIA02 " & _
             '               "    WHERE "&permi & _
             '               "    ORDER BY     PERMISO , FECHASALIDA , PRODUCTO, CANTIDAD, OBSERV01, EJEC01, cantidadfac "
...
'                              +------------+
                ...
                Dim detallepermiso() ' vector temporal para dibujar el detalle de los permisos, es decir las diferentes refrencias en que fue utilizado


               Dim PermiDibu() 'Vector para almacenar los permisos que no estan vencidos y ya fueron dibujados en el excel
               Dim PermiVencidos() 'Vector utilizado para almacenar los permisos vencidos

               ReDim PermiDibu(2,1) ' Necesitamos el permiso, la descripcion
               ReDim PermiVencidos(13,1) ' Necesitamos todos los campos
                ...
                if not RsRepPermi.eof then
                 ...
                 IntBanFirma = 0
                 if not isnull( RsRepPermi.Fields.Item("refcia02").Value ) then
                    strSQLRefcia = " SELECT REFCIA01,             " & _
                                   "        NUMPED01,         " & _
                                   "        FIRMAE01,         " & _
                                   "        CVEPED01          " & _
                                   " FROM SSDAGI01                " & _
                                   " WHERE REFCIA01='" & RsRepPermi.Fields.Item("refcia02").Value  & "'" & _
                                   " UNION                        " & _
                                   " SELECT REFCIA01,         " & _
                                   "        NUMPED01,         " & _
                                   "        FIRMAE01,         " & _
                                   "        CVEPED01          " & _
                                   " FROM SSDAGE01            " & _
                                   " WHERE REFCIA01='"& RsRepPermi.Fields.Item("refcia02").Value & "'"

                    'Response.Write(strSQLRefcia)
                    'Response.End

                    Set RsRefcia = Server.CreateObject("ADODB.Recordset")
                    RsRefcia.ActiveConnection = MM_EXTRANET_STRING
    	   	          RsRefcia.Source = strSQLRefcia
    		            RsRefcia.CursorType = 0
    		            RsRefcia.CursorLocation = 2
      		          RsRefcia.LockType = 1
    	  	          RsRefcia.Open()
                    ...
                    if not RsRefcia.eof then
                            srtRefFrac   = LTRIM(RsRefcia.Fields.Item("REFCIA01").Value)
                            StrPedFrac   = LTRIM(RsRefcia.Fields.Item("NUMPED01").Value)
                            StrFirmaFrac = LTRIM(RsRefcia.Fields.Item("FIRMAE01").Value)
                            StrCvePeFrac = LTRIM(RsRefcia.Fields.Item("CVEPED01").Value)

                            if not isnull(StrFirmaFrac) and not isnull(StrCvePeFrac) then
                               if StrCvePeFrac <> "R1" then
                                 IntBanFirma = 1
                               end if
                            end if
                    end if
                    RsRefcia.close
                    Set RsRefcia = Nothing
                 else
                    IntBanFirma = 1
                    StrPedFrac   = ""
                 end if
                ...
                if IntBanFirma = 1 then

                    'Vamos a leer el primer registro
                    ReDim detallepermiso(22,intcontpermi)
                      ...
                      if not isnull(RsRepPermi.Fields.Item("PRODUCTO").Value) then
                         ...
                         strproducto= ltrim(RsRepPermi.Fields.Item("PRODUCTO").Value)
                      else
                      ...
                      if not isnull(RsRepPermi.Fields.Item("CANTIDADFAC").Value) then
                         ...
                         '' validar la unidad de medida de la fraccion
                         if not isnull(RsRepPermi.Fields.Item("refcia02").Value) then
                            if RsRepPermi.Fields.Item("refcia02").Value = "DAI06-7346" then
                               detallepermiso(18,intcontpermi-1) = CStr(RsRepPermi.Fields.Item("CANTIDADFAC").Value / 1000 )
                            else
                               if RsRepPermi.Fields.Item("refcia02").Value = "DAI06-9420" then
                                 detallepermiso(18,intcontpermi-1) = CStr(RsRepPermi.Fields.Item("CANTIDADFAC").Value / 1000 )
                               else
                         ...
                         'detallepermiso(4,intcontpermi-1)  = CStr(RsRepPermi.Fields.Item("CANTIDAD").Value- CDbl(detallepermiso(18,intcontpermi-1)) )' SALDO
                         detallepermiso(4,intcontpermi-1)  = CStr(RsRepPermi.Fields.Item("CANTIDAD").Value- CDbl(detallepermiso(18,intcontpermi-1)) )' SALDO
                      ...
                      detallepermiso(20,intcontpermi-1) = StrPedFrac
                    ...
                    '-------
                      if IntBanFirma = 1 then ' Fin del if de la firma electronica y R1
                       ...
                       '    if ltrim(detallepermiso(18,y) ) = ltrim(CStr(RsRepPermi.Fields.Item("CANTIDADFAC").Value)) then
                       '       quitarep = 1
                       '    end if
                       '  end if
                       'next
                      ...
                      if not isnull(RsRepPermi.Fields.Item("CANTIDAD").Value ) and not isnull( RsRepPermi.Fields.Item("CANTIDADFAC").Value ) then
                       if RsRepPermi.Fields.Item("CANTIDAD").Value  = RsRepPermi.Fields.Item("CANTIDADFAC").Value then
                          quitarep = 1
                       end if
                      end if


                      if( detallepermiso(0,intcontpermi -2) = ltrim(RsRepPermi.Fields.Item("PERMISO").Value ) and detallepermiso(9,intcontpermi -2)  = CStr(RsRepPermi.Fields.Item("FECHASALIDA").Value ) and detallepermiso(1,intcontpermi -2)  = ltrim(RsRepPermi.Fields.Item("PRODUCTO").Value )    and detallepermiso(2,intcontpermi -2) = CStr(RsRepPermi.Fields.Item("CANTIDAD").Value ) and detallepermiso(15,intcontpermi-2) = CStr(RsRepPermi.Fields.Item("EJEC01").Value )  and detallepermiso(16,intcontpermi-2) = CStr(RsRepPermi.Fields.Item("OBSERV01").Value ) and detallepermiso(6,intcontpermi -2) = CStr(RsRepPermi.Fields.Item("PRESENT01").Value ) and detallepermiso(17,intcontpermi-2) = ltrim(RsRepPermi.Fields.Item("refcia02").Value)  )  then
                          if quitarep = 1  then
                             intcontpermi = intcontpermi - 1
                       ...
                       'if  quitarep = 1       then 'eliminamos el anterior
                       '   intcontpermi = intcontpermi - 1
                       'end if
                      ...
                      if (banentrar = 1 ) then
                        ...
                        if strcompermi= ltrim(RsRepPermi.Fields.Item("PERMISO").Value) and strObservaciones=ltrim(RsRepPermi.Fields.Item("OBSERV01").Value) and strproducto= ltrim(RsRepPermi.Fields.Item("PRODUCTO").Value) then 'es el mismo permiso
                            'ReDim detallepermiso(intcontpermi,20)


                            ReDim PRESERVE detallepermiso(22,intcontpermi)
                            if not isnull(RsRepPermi.Fields.Item("PERMISO").Value) then
                               strcompermi                     = ltrim(RsRepPermi.Fields.Item("PERMISO").Value)
                               detallepermiso(0,intcontpermi-1)  = ltrim(RsRepPermi.Fields.Item("PERMISO").Value)
                            else
                            ...
                            '--------------------------------------------------------------------------------------------
                             ...
                             for x=0 to ( UBound(detallepermiso,2)-1 )
                                ...
                                StrPed_aux         = detallepermiso(20,x) 'Pedimento
...
'                              intDiasVenci   = detallepermiso(x,14) 'DIASVEN01
                              ...
                              if dFechaVenci < dfechaactual then ' tiene una fecha vencida
                                  ...
                                  With oSS.ActiveSheet
                                      ...
                                      contpermiVen = UBound(PermiVencidos,2) - 1
                                      PermiVencidos(0,contpermiVen)   =  strProducto
                                      PermiVencidos(1,contpermiVen)   =  intCantidad
                                      PermiVencidos(2,contpermiVen)   =  intUtilizada2
                                      PermiVencidos(3,contpermiVen)   =  intSaldo
                                      PermiVencidos(4,contpermiVen)   =  strUniMed
                                      PermiVencidos(5,contpermiVen)   =  strregsan
                                      PermiVencidos(6,contpermiVen)   =  strPermi
                                      PermiVencidos(7,contpermiVen)   =  FormatDateTime(dFechaSali, vbGeneralDate)
                                      PermiVencidos(8,contpermiVen)   =  FormatDateTime(dFechaVenci, vbGeneralDate)
                                      PermiVencidos(9,contpermiVen)   =  strPaisFabric&","&strPaisFactur&","&strPaisProv
                                      PermiVencidos(10,contpermiVen)  =  strObserv
                                      PermiVencidos(11,contpermiVen)  =  strReferencia
                                      PermiVencidos(12,contpermiVen)  =  strEjecu
                                      PermiVencidos(13,contpermiVen)  =  StrPed_aux 'PEDIMENTO

                                      ReDim PRESERVE PermiVencidos(13,contpermiVen + 2)

                                      'ReDim PermiDibu(2,1) ' Necesitamos el permiso, la descripcion
                                      'ReDim PermiVencidos(20,1) ' Necesitamos todos los campos
                                   ...
                                   if (dblSumCantidadUtil)>=(intCantidad) then
                                       ...
                                       For intRow = 1 To Intnumejec
                                           IF oSS.Worksheets(intRow).Name  = LTRIM(strEjecu) THEN
                                               ...
                                               With oSS.ActiveSheet
                                                   ...
                                                   banColor = 0
                                                   if isdate(dFechaVenci) then
                                                     dFechV = DateValue(dFechaVenci)
                                                     if dFechV <= (Date() + 60) then' fecha actual mas dos meses
                                                       banColor = 1
                                                     end if
                                                   end if

                                                   if banColor = 1 then
                                                     intColorxVencer = 36 'Amarillo
                                                   else
                                                     intColorxVencer = 2 'Blanco
                                                   end if

                                                   .Cells(contadorPag(intRow - 1), 2).Value  = strProducto
                                                   .Cells(contadorPag(intRow - 1), 2).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 2).Interior.ColorIndex = intColorxVencer
                                                   .Cells(contadorPag(intRow - 1), 3).Value  = intCantidad
                                                   .Cells(contadorPag(intRow - 1), 3).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 3).Interior.ColorIndex = intColorxVencer
                                                   .Cells(contadorPag(intRow - 1), 4).Value  = intUtilizada2
                                                   .Cells(contadorPag(intRow - 1), 4).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 4).Interior.ColorIndex = intColorxVencer
                                                   .Cells(contadorPag(intRow - 1), 5).Value  = intSaldo
                                                   .Cells(contadorPag(intRow - 1), 5).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 5).Interior.ColorIndex = intColorxVencer
                                                   .Cells(contadorPag(intRow - 1), 6).Value  = StrPed_aux 'Pedimento
                                                   .Cells(contadorPag(intRow - 1), 6).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 6).Interior.ColorIndex = intColorxVencer
                                                   .Cells(contadorPag(intRow - 1), 7).Value  = strUniMed
                                                   .Cells(contadorPag(intRow - 1), 7).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 7).Interior.ColorIndex = intColorxVencer
                                                   .Cells(contadorPag(intRow - 1), 8).Value  = strregsan
                                                   .Cells(contadorPag(intRow - 1), 8).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 8).Interior.ColorIndex = intColorxVencer
                                                   .Cells(contadorPag(intRow - 1), 9).Value  = strPermi
                                                   .Cells(contadorPag(intRow - 1), 9).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 9).Interior.ColorIndex = intColorxVencer
                                                   .Cells(contadorPag(intRow - 1), 10).Value  = FormatDateTime(dFechaSali, vbGeneralDate)
                                                   .Cells(contadorPag(intRow - 1), 10).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 10).Interior.ColorIndex = intColorxVencer
                                                   .Cells(contadorPag(intRow - 1), 11).Value = FormatDateTime(dFechaVenci, vbGeneralDate)
                                                   .Cells(contadorPag(intRow - 1), 11).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 11).Interior.ColorIndex = intColorxVencer
                                                   .Cells(contadorPag(intRow - 1), 12).Value = strPaisFabric&","&strPaisFactur&","&strPaisProv
                                                   .Cells(contadorPag(intRow - 1), 12).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 12).Interior.ColorIndex = intColorxVencer
                                                   .Cells(contadorPag(intRow - 1), 13).Value = ""
                                                   '.Cells(contadorPag(intRow - 1), 12).Value = strObserv
                                                   .Cells(contadorPag(intRow - 1), 13).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 13).Interior.ColorIndex = intColorxVencer
                                                   .Cells(contadorPag(intRow - 1), 14).Value = strReferencia
                                                   .Cells(contadorPag(intRow - 1), 14).Font.size = 8
                                                   .Cells(contadorPag(intRow - 1), 14).Interior.ColorIndex = intColorxVencer

                                                   contadorPag(intRow - 1) = contadorPag(intRow - 1) + 1

                                                   contpermiDib = UBound(PermiDibu,2) - 1
                                                   PermiDibu(0,contpermiDib) = strPermi
                                                   PermiDibu(1,contpermiDib) = strProducto
                                                   ReDim PRESERVE PermiDibu(2,contpermiDib + 2)
                                       ...
                                       if intbanEjec = 0 then ' Para la hoja de muestras
                                           ...
                                           With oSS.ActiveSheet
                                              ...
                                              .Cells(contadorPag(intcontEjec-1), 2).Value  = strProducto
                                              .Cells(contadorPag(intcontEjec-1), 2).Interior.ColorIndex = intColorxVencer
                                              .Cells(contadorPag(intcontEjec-1), 3).Value  = intCantidad
                                              .Cells(contadorPag(intcontEjec-1), 3).Interior.ColorIndex = intColorxVencer
                                              .Cells(contadorPag(intcontEjec-1), 3).Font.size = 8
                                              .Cells(contadorPag(intcontEjec-1), 4).Value  = intUtilizada2
                                              .Cells(contadorPag(intcontEjec-1), 4).Interior.ColorIndex = intColorxVencer
                                              .Cells(contadorPag(intcontEjec-1), 4).Font.size = 8
                                              .Cells(contadorPag(intcontEjec-1), 5).Value  = intSaldo
                                              .Cells(contadorPag(intcontEjec-1), 5).Interior.ColorIndex = intColorxVencer
                                              .Cells(contadorPag(intcontEjec-1), 5).Font.size = 8
                                              .Cells(contadorPag(intcontEjec-1), 6).Value  = StrPed_aux 'Pedimento
                                              .Cells(contadorPag(intcontEjec-1), 6).Interior.ColorIndex = intColorxVencer
                                              .Cells(contadorPag(intcontEjec-1), 6).Font.size = 8
                                              .Cells(contadorPag(intcontEjec-1), 7).Value  = strUniMed
                                              .Cells(contadorPag(intcontEjec-1), 7).Interior.ColorIndex = intColorxVencer
                                              .Cells(contadorPag(intcontEjec-1), 7).Font.size = 8
                                              .Cells(contadorPag(intcontEjec-1), 8).Value  = strregsan
                                              .Cells(contadorPag(intcontEjec-1), 8).Interior.ColorIndex = intColorxVencer
                                              .Cells(contadorPag(intcontEjec-1), 8).Font.size = 8
                                              .Cells(contadorPag(intcontEjec-1), 9).Value  = strPermi
                                              .Cells(contadorPag(intcontEjec-1), 9).Interior.ColorIndex = intColorxVencer
                                              .Cells(contadorPag(intcontEjec-1), 9).Font.size = 8
                                              .Cells(contadorPag(intcontEjec-1), 10).Value  = FormatDateTime(dFechaSali, vbGeneralDate)
                                              .Cells(contadorPag(intcontEjec-1), 10).Interior.ColorIndex = intColorxVencer
                                              .Cells(contadorPag(intcontEjec-1), 10).Font.size = 8
                                              .Cells(contadorPag(intcontEjec-1), 11).Value = FormatDateTime(dFechaVenci, vbGeneralDate)
                                              .Cells(contadorPag(intcontEjec-1), 11).Interior.ColorIndex = intColorxVencer
                                              .Cells(contadorPag(intcontEjec-1), 11).Font.size = 8
                                              .Cells(contadorPag(intcontEjec-1), 12).Value = strPaisFabric&","&strPaisFactur&","&strPaisProv
                                              .Cells(contadorPag(intcontEjec-1), 12).Interior.ColorIndex = intColorxVencer
                                              .Cells(contadorPag(intcontEjec-1), 12).Font.size = 8
                                              '.Cells(contadorPag(intcontEjec-1), 12).Value = strObserv
                                              .Cells(contadorPag(intcontEjec-1), 13).Value = ""
                                              .Cells(contadorPag(intcontEjec-1), 13).Interior.ColorIndex = intColorxVencer
                                              .Cells(contadorPag(intcontEjec-1), 13).Font.size = 8
                                              .Cells(contadorPag(intcontEjec-1), 14).Value = strReferencia
                                              .Cells(contadorPag(intcontEjec-1), 14).Font.size = 8
                                              .Cells(contadorPag(intcontEjec-1), 14).Interior.ColorIndex = intColorxVencer
                     ...
                     end if ' Fin del if de la firma electronica y R1
...
'--------------------------------------------------------------------------------------------------------------------------------------------
          ...
          'Response.Write( UBound(PermiVencidos,2) )
          'Response.End


             '' ---------------------------------------------------------------------------------------''
             ' Dibujamos los vencidos
             oSS.Worksheets(intcontEjec).Activate
             With oSS.ActiveSheet


             y=0
             strNombre = ltrim(PermiVencidos(0,y) )
             while y < ( UBound(PermiVencidos,2) - 2)

                  banPermiDib = 0
                  for x=0 to ( UBound(PermiDibu,2) - 2)
                     if ( ltrim(PermiDibu(1,x) ) = ltrim(PermiVencidos(0,y) )  ) then
                         banPermiDib = 1
                     end if
                  next

                  if banPermiDib = 0 then  'No se ha dibujado esta mercancias
                     while strNombre = ltrim(PermiVencidos(0,y)) and y < ( UBound(PermiVencidos,2) - 2)

                      intbanEjec = 0
                      For intRow = 1 To Intnumejec
                        IF oSS.Worksheets(intRow).Name  = LTRIM( PermiVencidos(12,y) ) THEN
                           intbanEjec = 1
                           oSS.Worksheets(intRow).Activate
                           With oSS.ActiveSheet
                                .Cells(contadorPag(intRow - 1), 2).Value  = PermiVencidos(0,y)
                                .Cells(contadorPag(intRow - 1), 2).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 2).Interior.ColorIndex = 3
                                .Cells(contadorPag(intRow - 1), 3).Value  = PermiVencidos(1,y)
                                .Cells(contadorPag(intRow - 1), 3).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 3).Interior.ColorIndex = 3
                                .Cells(contadorPag(intRow - 1), 4).Value  = PermiVencidos(2,y)
                                .Cells(contadorPag(intRow - 1), 4).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 4).Interior.ColorIndex = 3
                                .Cells(contadorPag(intRow - 1), 5).Value  = PermiVencidos(3,y)
                                .Cells(contadorPag(intRow - 1), 5).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 5).Interior.ColorIndex = 3
                                .Cells(contadorPag(intRow - 1), 6).Value  = PermiVencidos(13,y) 'Pedimento
                                .Cells(contadorPag(intRow - 1), 6).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 6).Interior.ColorIndex = 3
                                .Cells(contadorPag(intRow - 1), 7).Value  = PermiVencidos(4,y)
                                .Cells(contadorPag(intRow - 1), 7).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 7).Interior.ColorIndex = 3
                                .Cells(contadorPag(intRow - 1), 8).Value  = PermiVencidos(5,y)
                                .Cells(contadorPag(intRow - 1), 8).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 8).Interior.ColorIndex = 3
                                .Cells(contadorPag(intRow - 1), 9).Value  = PermiVencidos(6,y)
                                .Cells(contadorPag(intRow - 1), 9).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 9).Interior.ColorIndex = 3
                                .Cells(contadorPag(intRow - 1), 10).Value  = PermiVencidos(7,y)
                                .Cells(contadorPag(intRow - 1), 10).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 10).Interior.ColorIndex = 3
                                .Cells(contadorPag(intRow - 1), 11).Value = PermiVencidos(8,y)
                                .Cells(contadorPag(intRow - 1), 11).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 11).Interior.ColorIndex = 3
                                .Cells(contadorPag(intRow - 1), 12).Value = PermiVencidos(9,y)
                                .Cells(contadorPag(intRow - 1), 12).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 12).Interior.ColorIndex = 3
                                '.Cells(contadorPag(intRow - 1), 12).Value = PermiVencidos(10,y) 'Observaciones
                                .Cells(contadorPag(intRow - 1), 13).Value = ""
                                .Cells(contadorPag(intRow - 1), 13).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 13).Interior.ColorIndex = 3
                                .Cells(contadorPag(intRow - 1), 14).Value = PermiVencidos(11,y)
                                .Cells(contadorPag(intRow - 1), 14).Font.size = 8
                                .Cells(contadorPag(intRow - 1), 14).Interior.ColorIndex = 3
                     ...
                     wend
                      ...
                      contpermiDib = UBound(PermiDibu,2) - 1
                      PermiDibu(0,contpermiDib) = PermiVencidos(6,y-1)
                      PermiDibu(1,contpermiDib) = PermiVencidos(0,y-1)
                      ReDim PRESERVE PermiDibu(2,contpermiDib + 2)


                  else ' Ya hay un permiso mas reciente

                     while strNombre = ltrim(PermiVencidos(0,y) )and y < ( UBound(PermiVencidos,2) - 2)
                        .Cells(contadorPag(intcontEjec-1), 2).Value  = PermiVencidos(0,y)
                        .Cells(contadorPag(intcontEjec-1), 2).Interior.ColorIndex = 3
                        .Cells(contadorPag(intcontEjec-1), 2).Font.size = 8
                        .Cells(contadorPag(intcontEjec-1), 3).Value  = PermiVencidos(1,y)
                        .Cells(contadorPag(intcontEjec-1), 3).Interior.ColorIndex = 3
                        .Cells(contadorPag(intcontEjec-1), 3).Font.size = 8
                        .Cells(contadorPag(intcontEjec-1), 4).Value  = PermiVencidos(2,y)
                        .Cells(contadorPag(intcontEjec-1), 4).Interior.ColorIndex = 3
                        .Cells(contadorPag(intcontEjec-1), 4).Font.size = 8
                        .Cells(contadorPag(intcontEjec-1), 5).Value  = PermiVencidos(3,y)
                        .Cells(contadorPag(intcontEjec-1), 5).Interior.ColorIndex = 3
                        .Cells(contadorPag(intcontEjec-1), 5).Font.size = 8

                        .Cells(contadorPag(intcontEjec-1), 6).Value  = PermiVencidos(13,y)
                        .Cells(contadorPag(intcontEjec-1), 6).Font.size = 8
                        .Cells(contadorPag(intcontEjec-1), 6).Interior.ColorIndex = 3


                        .Cells(contadorPag(intcontEjec-1), 7).Value  = PermiVencidos(4,y)
                        .Cells(contadorPag(intcontEjec-1), 7).Interior.ColorIndex = 3
                        .Cells(contadorPag(intcontEjec-1), 7).Font.size = 8
                        .Cells(contadorPag(intcontEjec-1), 8).Value  = PermiVencidos(5,y)
                        .Cells(contadorPag(intcontEjec-1), 8).Interior.ColorIndex = 3
                        .Cells(contadorPag(intcontEjec-1), 8).Font.size = 8
                        .Cells(contadorPag(intcontEjec-1), 9).Value  = PermiVencidos(6,y)
                        .Cells(contadorPag(intcontEjec-1), 9).Interior.ColorIndex = 3
                        .Cells(contadorPag(intcontEjec-1), 9).Font.size = 8
                        .Cells(contadorPag(intcontEjec-1), 10).Value  = PermiVencidos(7,y)
                        .Cells(contadorPag(intcontEjec-1), 10).Interior.ColorIndex = 3
                        .Cells(contadorPag(intcontEjec-1), 10).Font.size = 8
                        .Cells(contadorPag(intcontEjec-1), 11).Value = PermiVencidos(8,y)
                        .Cells(contadorPag(intcontEjec-1), 11).Interior.ColorIndex = 3
                        .Cells(contadorPag(intcontEjec-1), 11).Font.size = 8
                        .Cells(contadorPag(intcontEjec-1), 12).Value = PermiVencidos(9,y)
                        .Cells(contadorPag(intcontEjec-1), 12).Interior.ColorIndex = 3
                        .Cells(contadorPag(intcontEjec-1), 12).Font.size = 8
                        '.Cells(contadorPag(intcontEjec-1), 12).Value = PermiVencidos(10,y)
                        .Cells(contadorPag(intcontEjec-1), 13).Value = ""
                        .Cells(contadorPag(intcontEjec-1), 13).Interior.ColorIndex = 3
                        .Cells(contadorPag(intcontEjec-1), 13).Font.size = 8
                        .Cells(contadorPag(intcontEjec-1), 14).Value = PermiVencidos(11,y)
                        .Cells(contadorPag(intcontEjec-1), 14).Interior.ColorIndex = 3
                        .Cells(contadorPag(intcontEjec-1), 14).Font.size = 8
                        contadorPag(intcontEjec-1) = contadorPag(intcontEjec-1) + 1
                        y = y + 1
                     wend
