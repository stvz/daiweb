...
class Exportaciones(Base):
    ...
    def getReferencia(self,_referencia):
        ...
        referencia_ = self.dictResult(_consulta="select * from ssdage01 where refcia01 = '{0}'".format(_referencia))
        referencia_
        self.cerrarODBC()
        return referencia_
    ...
    def getReferencias(self, _referencias ):
        ...
        if isinstance(_referencias,list):
            consulta_ = "select * from ssdage01 where refcia01 in ('{0}')".format("','".join(_referencias))
        elif isinstance(_referencias,str):
            consulta_ = "select * from ssdage01 where refcia01 in ('{0}')".format("','".join(_referencias.split(',')))
        else:
