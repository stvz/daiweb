...
class Referencias(Base):
    ...
    def getReferencias(self, _referencias ):
        ...
        if isinstance(_referencias,list):
            consulta_ = "select * from c01refer where refe01 in ('{0}')".format("','".join(_referencias))
        elif isinstance(_referencias,str):
            consulta_ = "select * from c01refer where refe01 in ('{0}')".format("','".join(_referencias.split(',')))
        else:
